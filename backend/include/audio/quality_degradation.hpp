...
public:
     ...
     */
    void registerQualityChangeCallback(
...
private:
    ...
    std::vector<std::function<void(const AudioQualityParams&, const AudioQualityParams&)>> 
    ...
    // History
    std::vector<std::pair<std::chrono::steady_clock::time_point, AudioQualityParams>> 
        qualityHistory_;
    
    // Private methods
    AudioQualityParams calculateNetworkOptimizedParams(NetworkQuality quality,
                                                      const NetworkMetrics& metrics) const;
    AudioQualityParams calculateResourceOptimizedParams(float cpuUsage, float memoryUsage,
                                                       float latency) const;
    ...
    AudioQualityLevel getQualityLevelFromParams(const AudioQualityParams& params) const;
...
public:
     ...
     * @param processingLatency Current processing latency in ms
     */
    void updateSystemResources(float cpuUsage, float memoryUsage, float processingLatency);
...
private:
    ...
    bool shouldAdjustForNetwork(NetworkQuality quality, const NetworkMetrics& metrics) const;
