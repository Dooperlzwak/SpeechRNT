...
private:
    ...
    void updateStatistics(bool usedSilero, float confidence, double processingTimeMs);
    
    // Audio preprocessing
    std::vector<float> preprocessAudio(const std::vector<float>& samples);
    std::vector<float> resampleIfNeeded(const std::vector<float>& samples, uint32_t targetSampleRate);
};
...
public:
    ...
    explicit EnergyBasedVAD(const Config& config = Config{});
