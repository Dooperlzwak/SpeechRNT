...
struct AdvancedStreamingConfig {
    ...
    AdvancedStreamingConfig()
        ...
        , enableQualityDegradation(true), cpuThreshold(0.8f), memoryThreshold(0.8f)
        , enableLoadBalancing(true), numWorkerThreads(4), maxQueueSize(1000)
        , targetLatencyMs(200), enableUltraLowLatency(true) {}
};
...
struct StreamingPerformanceMetrics {
    ...
    StreamingPerformanceMetrics()
        : endToEndLatencyMs(0.0f), networkLatencyMs(0.0f), processingLatencyMs(0.0f)
        , queueLatencyMs(0.0f), packetLossRate(0.0f), throughputMbps(0.0f)
        , cpuUsage(0.0f), memoryUsage(0.0f), activeStreams(0), queuedJobs(0)
...
public:
     ...
     */
    uint64_t submitRealTimeJob(std::function<void()> task, 
     ...
     * @return job ID for tracking
     */
    uint64_t submitBatchJob(std::function<void()> task,
     ...
     */
    void registerMetricsCallback(std::function<void(const StreamingPerformanceMetrics&)> callback);
...
private:
    ...
    std::vector<std::function<void(const StreamingPerformanceMetrics&)>> metricsCallbacks_;
    ...
    void onQualityChange(const AudioQualityParams& oldParams, const AudioQualityParams& newParams);
...
public:
     ...
     */
    size_t addChunkIntelligent(const AudioChunk& chunk, std::vector<AudioChunk>& orderedChunks);
