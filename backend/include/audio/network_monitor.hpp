...
public:
   ...
   */
  void registerConditionCallback(
...
private:
  ...
  // Callbacks
  std::vector<std::function<void(const NetworkMetrics &, NetworkQuality)>>
      conditionCallbacks_;
  ...
  void notifyConditionChange(const NetworkMetrics &metrics,
...
struct AdaptiveStreamingParams {
  ...
  AdaptiveStreamingParams()
      : bufferSizeMs(100), chunkSizeMs(50), maxRetries(3), qualityFactor(1.0f),
        enableCompression(true), timeoutMs(5000) {}
};
...
private:
  ...
  // Adaptation history
  std::vector<
      std::pair<std::chrono::steady_clock::time_point, AdaptiveStreamingParams>>
      adaptationHistory_;
  ...
  void onNetworkConditionChange(const NetworkMetrics &metrics,
                                NetworkQuality quality);
  AdaptiveStreamingParams calculateOptimalParams(const NetworkMetrics &metrics,
