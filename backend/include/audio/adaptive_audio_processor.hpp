...
struct AdaptiveProcessingParams {
  ...
  float
      spectralSubtractionAlpha; // Spectral subtraction over-subtraction factor
  ...
  AdaptiveProcessingParams()
      : noiseReductionStrength(0.5f), spectralSubtractionAlpha(2.0f),
        wienerFilterBeta(0.1f), targetRMS(0.1f), compressionRatio(2.0f),
        attackTime(0.01f), releaseTime(0.1f), echoSuppressionStrength(0.7f),
        adaptiveFilterLength(512), convergenceRate(0.01f), selectedChannel(-1),
        channelMixingWeight(0.5f), enableChannelSelection(true),
        enablePreEmphasis(false), preEmphasisCoeff(0.97f),
        enablePostProcessing(true) {}
};

// Audio type classification for processing optimization
enum class AudioType { UNKNOWN, SPEECH, MUSIC, NOISE, MIXED, SILENCE };
...
struct AudioCharacteristics {
  ...
  AudioCharacteristics()
      : type(AudioType::UNKNOWN), speechProbability(0.0f),
        musicProbability(0.0f), noiseProbability(0.0f), reverbLevel(0.0f),
        backgroundNoiseLevel(0.0f), dynamicRange(0.0f), recommendedChannel(-1),
        stationarity(0.0f), periodicityStrength(0.0f) {}
};
...
struct QualityMonitoringResult {
  ...
  QualityMonitoringResult() : parametersChanged(false) {
...
public:
  ...
  processMultiChannelAudio(const std::vector<std::vector<float>> &audioData);
  ...
  AudioCharacteristics
  analyzeAudioCharacteristics(const std::vector<float> &audioData);
  AudioType classifyAudioType(const std::vector<float> &audioData);

  // Quality monitoring
  void enableQualityMonitoring(bool enabled) {
    qualityMonitoringEnabled_ = enabled;
  }
  QualityMonitoringResult getLatestQualityReport() const;
  std::vector<QualityMonitoringResult>
  getQualityHistory(size_t maxEntries = 100) const;
  ...
  std::vector<float>
  mixChannels(const std::vector<std::vector<float>> &audioData,
...
private:
  ...
  std::vector<float>
  selectChannelIntelligently(const std::vector<std::vector<float>> &audioData);
  std::vector<float>
  evaluateChannelQuality(const std::vector<float> &channelData);
...
public:
  ...
  std::vector<float>
  processSpectralSubtraction(const std::vector<float> &audioData, float alpha);
...
public:
  ...
  std::vector<float>
  selectBestChannel(const std::vector<std::vector<float>> &audioData);
  std::vector<float>
  mixChannelsAdaptively(const std::vector<std::vector<float>> &audioData);
  std::vector<float>
  evaluateChannelQualities(const std::vector<std::vector<float>> &audioData);
...
public:
  ...
  std::vector<float>
  calculateTypeProbabilities(const std::vector<float> &audioData);
