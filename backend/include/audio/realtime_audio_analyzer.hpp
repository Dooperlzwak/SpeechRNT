...
struct SpectralAnalysis {
    ...
    SpectralAnalysis() 
        : dominantFrequency(0.0f), spectralCentroid(0.0f), spectralBandwidth(0.0f),
          spectralRolloff(0.0f), spectralFlatness(0.0f), spectralFlux(0.0f) {
...
struct AudioEffectsConfig {
    ...
    AudioEffectsConfig()
        : enableCompressor(false), enableEqualizer(false), enableReverb(false), enableNoiseGate(false),
          compressorThreshold(-20.0f), compressorRatio(4.0f), compressorAttack(5.0f), compressorRelease(50.0f),
...
public:
    ...
    // Windowing functions
    void setWindowFunction(const std::string& windowType); // "hann", "hamming", "blackman", "rectangular"
...
public:
    explicit RealTimeAudioAnalyzer(uint32_t sampleRate = 16000, size_t bufferSize = 1024);
...
private:
    ...
    void updateSpeechDetection(const AudioLevelMetrics& levels, const SpectralAnalysis& spectral);
    ...
    float calculateSpectralCentroid(const std::vector<float>& spectrum, uint32_t sampleRate);
    float calculateSpectralBandwidth(const std::vector<float>& spectrum, float centroid, uint32_t sampleRate);
    float calculateSpectralRolloff(const std::vector<float>& spectrum, uint32_t sampleRate, float rolloffPercent = 0.85f);
    ...
    std::vector<float> calculateMFCC(const std::vector<float>& spectrum, uint32_t sampleRate);
    float estimateNoiseLevel(const std::vector<float>& samples);
    float calculateSpeechProbability(const AudioLevelMetrics& levels, const SpectralAnalysis& spectral);
