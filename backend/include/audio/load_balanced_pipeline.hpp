...
struct ProcessingJob {
  ...
  ProcessingJob(uint64_t id, ProcessingJobType jobType, ProcessingPriority prio,
      ...
      : jobId(id), type(jobType), priority(prio), task(taskFunc),
        description(desc), submissionTime(std::chrono::steady_clock::now()),
        deadline(std::chrono::steady_clock::now() + std::chrono::seconds(30)),
        estimatedProcessingTimeMs(100), resourceRequirement(5) {}
};
...
struct SystemResources {
  ...
  SystemResources()
      : cpuUsage(0.0f), memoryUsage(0.0f), gpuUsage(0.0f), activeThreads(0),
        queuedJobs(0), averageLatency(0.0f), resourceConstrained(false) {}
};
...
public:
   ...
   * @return true if job is available
   */
  bool getNextJobWithPriority(ProcessingJob &job,
...
public:
   ...
   * @param priority Priority level
   */
  void setJobTypePriority(ProcessingJobType jobType,
                          ProcessingPriority priority);
   ...
   */
  void setResourceThresholds(float cpuThreshold = 0.8f,
...
private:
  ...
  void updateStatistics(const ProcessingJob &job, float processingTime,
                        float queueTime);
...
public:
   ...
   */
  static std::unique_ptr<LoadBalancedProcessingPipeline>
  createRealTimePipeline(size_t numThreads = 2);
   ...
   */
  static std::unique_ptr<LoadBalancedProcessingPipeline>
  createBatchPipeline(size_t numThreads = 8);
   ...
   */
  static std::unique_ptr<LoadBalancedProcessingPipeline>
  createHybridPipeline(size_t numThreads = 4);
   ...
   */
  static std::unique_ptr<LoadBalancedProcessingPipeline>
  createCustomPipeline(const std::map<std::string, std::string> &config);
};
