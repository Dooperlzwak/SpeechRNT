...
struct SynthesisResult {
    ...
    SynthesisResult() : duration(0.0f), sampleRate(22050), channels(1), success(false) {}
};
...
public:
     ...
     */
    virtual bool initialize(const std::string& modelPath, const std::string& voiceId = "") = 0;
     ...
     */
    virtual SynthesisResult synthesize(const std::string& text, const std::string& voiceId = "") = 0;
     ...
     */
    virtual std::future<SynthesisResult> synthesizeAsync(const std::string& text, const std::string& voiceId = "") = 0;
     ...
     * @param voiceId Voice ID to use (empty for default)
     */
    virtual void synthesizeWithCallback(const std::string& text, SynthesisCallback callback, const std::string& voiceId = "") = 0;
     ...
     */
    virtual std::vector<VoiceInfo> getVoicesForLanguage(const std::string& language) const = 0;
     ...
     */
    virtual void setSynthesisParameters(float speed = 1.0f, float pitch = 0.0f, float volume = 1.0f) = 0;
