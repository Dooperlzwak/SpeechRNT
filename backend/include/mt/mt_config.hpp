...
/**
 * Configuration structure for model families
 */
struct ModelFamilyConfig {
  ...
  ModelFamilyConfig() : enabled(false), isDefault(false) {}
};
...
struct GPUConfig {
  ...
  GPUConfig()
      : enabled(true), fallbackToCPU(true), defaultDeviceId(0),
        memoryPoolSizeMB(1024), maxModelMemoryMB(2048),
        memoryReservationRatio(0.8f) {}
};
...
struct QualityConfig {
  ...
  QualityConfig()
      : enabled(true), highQualityThreshold(0.8f), mediumQualityThreshold(0.6f),
        lowQualityThreshold(0.4f), generateAlternatives(true),
        maxAlternatives(3), enableFallbackTranslation(true) {}
};
...
struct CachingConfig {
  ...
  CachingConfig()
      : enabled(true), maxCacheSize(1000),
        cacheExpirationTime(std::chrono::minutes(60)), persistToDisk(false),
...
struct StreamingConfig {
  ...
  StreamingConfig()
      : enabled(true), sessionTimeout(std::chrono::minutes(30)),
        maxConcurrentSessions(100), maxContextLength(1000),
...
struct ErrorHandlingConfig {
  ...
  ErrorHandlingConfig()
      : enableRetry(true), maxRetryAttempts(3),
        initialRetryDelay(std::chrono::milliseconds(100)),
        retryBackoffMultiplier(2.0f),
        maxRetryDelay(std::chrono::milliseconds(10000)),
        translationTimeout(std::chrono::milliseconds(5000)),
        enableDegradedMode(true), enableFallbackTranslation(true) {}
};
...
struct LanguageDetectionConfig {
  ...
  LanguageDetectionConfig()
      : enabled(true), confidenceThreshold(0.7f), detectionMethod("hybrid"),
        enableHybridDetection(true), hybridWeightText(0.6f),
        hybridWeightAudio(0.4f) {}
};
...
public:
  ...
  bool updateModelConfig(const std::string &sourceLang,
  ...
  void setCustomModelPath(const std::string &sourceLang,
                          const std::string &targetLang,
                          const std::string &modelPath);
  std::string getModelPath(const std::string &sourceLang,
                           const std::string &targetLang) const;
  bool hasCustomModelPath(const std::string &sourceLang,
                          const std::string &targetLang) const;

  // Model Families
  const std::unordered_map<std::string, ModelFamilyConfig> &
  getModelFamilies() const {
    return modelFamilies_;
  }

  // Language pair configuration
  void addLanguagePair(const std::string &sourceLang,
                       const std::string &targetLang,
                       const MarianModelConfig &config);
  void removeLanguagePair(const std::string &sourceLang,
                          const std::string &targetLang);
  bool hasLanguagePair(const std::string &sourceLang,
                       const std::string &targetLang) const;
  MarianModelConfig getModelConfig(const std::string &sourceLang,
                                   const std::string &targetLang) const;
  std::vector<std::pair<std::string, std::string>>
  getAvailableLanguagePairs() const;

  // Supported languages
  std::vector<std::string> getSupportedSourceLanguages() const;
  std::vector<std::string>
  getSupportedTargetLanguages(const std::string &sourceLang) const;

  // Configuration change notifications
  using ConfigChangeCallback =
      std::function<void(const std::string &section, const std::string &key)>;
  void registerConfigChangeCallback(const std::string &callbackId,
  ...
  const ErrorHandlingConfig &getErrorHandlingConfig() const {
    return errorHandlingConfig_;
  }
  const PerformanceConfig &getPerformanceConfig() const {
    return performanceConfig_;
  }
  const LanguageDetectionConfig &getLanguageDetectionConfig() const {
    return languageDetectionConfig_;
  }
...
private:
  ...
  std::unordered_map<std::string, ModelFamilyConfig>
      modelFamilies_; // Added this line
  ...
  std::string getLanguagePairKey(const std::string &sourceLang,
                                 const std::string &targetLang) const;
