...
struct QualityMetrics {
  float bleuScore = 0.0f;
  float meteorScore = 0.0f;
...
struct TranslationResult {
  ...
  TranslationResult()
      : confidence(0.0f), success(false), processingTime(0),
        usedGPUAcceleration(false), batchIndex(-1), isPartialResult(false),
...
public:
   ...
   */
  virtual bool initialize(const std::string &sourceLang,
   ...
   */
  virtual std::future<TranslationResult>
  translateAsync(const std::string &text) = 0;
   ...
   */
  virtual bool supportsLanguagePair(const std::string &sourceLang,
                                    const std::string &targetLang) const = 0;
  ...
  getSupportedTargetLanguages(const std::string &sourceLang) const = 0;
  ...
  translateBatch(const std::vector<std::string> &texts) = 0;
  ...
  translateBatchAsync(const std::vector<std::string> &texts) = 0;
   ...
   */
  virtual bool startStreamingTranslation(const std::string &sessionId,
   ...
   */
  virtual TranslationResult addStreamingText(const std::string &sessionId,
                                             const std::string &text,
                                             bool isComplete = false) = 0;
  ...
  finalizeStreamingTranslation(const std::string &sessionId) = 0;
