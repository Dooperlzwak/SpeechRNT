 ...
 */
using SessionRecoveryCallback = std::function<bool(
    const SessionRecoveryData &, std::shared_ptr<ClientSession>)>;
using SessionCleanupCallback =
...
public:
  explicit SessionRecoveryManager(
      const SessionRecoveryConfig &config = SessionRecoveryConfig{});
   ...
   */
  bool storeSessionData(const std::string &session_id,
  ...
  getSessionData(const std::string &session_id) const;
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<SessionRecoveryData>>
      stored_sessions_;
...
public:
   ...
   */
  static SessionRecoveryData
  createFromClientSession(std::shared_ptr<ClientSession> session);

  /**
   * Apply recovery data to client session
   */
  static bool applyToClientSession(const SessionRecoveryData &data,
   ...
   */
  static SessionRecoveryData
  mergeRecoveryData(const SessionRecoveryData &existing,
