...
private:
  ...
  void handleNewConnection(const std::string &sessionId,
                           uWS::WebSocket<false> *ws);
  ...
  void handleDetailedHealthCheck(uWS::HttpResponse<false> *res,
                                 uWS::HttpRequest *req);
  void handleHealthMetrics(uWS::HttpResponse<false> *res,
                           uWS::HttpRequest *req);
  void handleHealthHistory(uWS::HttpResponse<false> *res,
                           uWS::HttpRequest *req);
...
public:
  ...
  void sendBinaryMessage(const std::string &sessionId,
