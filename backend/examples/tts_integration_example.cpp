...
/**
 * Comprehensive example showing how to integrate TTS into the speech translation pipeline
 * Demonstrates best practices for TTS setup, configuration, and usage
 */
class TTSIntegrationExample {
public:
    TTSIntegrationExample() {
        std::cout << "=== TTS Integration Example ===" << std::endl;
        std::cout << "This example demonstrates how to integrate Coqui TTS into the speech translation pipeline." << std::endl;
        std::cout << std::endl;
    }
...
private:
    void initializeComponents() {
        ...
        utterance_manager_->setStateChangeCallback([](const core::UtteranceData& utterance) {
            std::cout << "📊 Utterance " << utterance.id << " state changed to: " 
                      << static_cast<int>(utterance.state) << std::endl;
        });
        
        utterance_manager_->setCompleteCallback([](const core::UtteranceData& utterance) {
            std::cout << "✅ Utterance " << utterance.id << " completed successfully" << std::endl;
        });
        
        utterance_manager_->setErrorCallback([](const core::UtteranceData& utterance, const std::string& error) {
            std::cout << "❌ Utterance " << utterance.id << " failed: " << error << std::endl;
        });
    }
    ...
    void setupTTSEngine() {
        ...
        // Create TTS engine
        tts_engine_ = tts::createCoquiTTS();
        if (!tts_engine_) {
        ...
        std::cout << "🔄 Initializing TTS engine with model path: " << model_path << std::endl;
        
        if (tts_engine_->initialize(model_path)) {
            std::cout << "✅ TTS engine initialized with real Coqui models" << std::endl;
        } else {
            std::cout << "⚠️  Real Coqui models not available, using mock implementation" << std::endl;
            std::cout << "   This is normal for development environments without TTS models" << std::endl;
        }
        ...
        if (tts_engine_->isReady()) {
            ...
            std::cout << "❌ TTS engine is not ready: " << tts_engine_->getLastError() << std::endl;
            return;
        }
    ...
    void demonstrateVoiceManagement() {
        ...
        for (const auto& lang : test_languages) {
            ...
            std::cout << "🌐 Voices for " << lang << ": " << lang_voices.size() << std::endl;
            for (const auto& voice : lang_voices) {
                std::cout << "    " << voice.id << " (" << voice.name << ", " << voice.gender << ")" << std::endl;
            }
        }
        ...
        if (!voices.empty()) {
            ...
            std::cout << "🎤 Current default voice: " << tts_engine_->getDefaultVoice() << std::endl;
        }
    }
    ...
    void testDirectSynthesis() {
        ...
        std::string test_text = "Hello, this is a test of the text-to-speech synthesis system.";\n        std::cout << "🔊 Synthesizing: \"" << test_text << "\"" << std::endl;
        ...
        if (result.success) {
            ...
            std::cout << "   Audio data: " << result.audioData.size() << " bytes" << std::endl;
            ...
            std::cout << "   Synthesis time: " << synthesis_time.count() << " ms" << std::endl;
        ...
        auto voices = tts_engine_->getAvailableVoices();
        if (voices.size() > 1) {
            std::cout << "\n🎤 Testing with specific voice: " << voices[1].id << std::endl;
            ...
            if (voice_result.success) {
                ...
                std::cout << "❌ Voice-specific synthesis failed: " << voice_result.errorMessage << std::endl;
            }
        }
        
        // Test asynchronous synthesis
        std::cout << "\n⏳ Testing asynchronous synthesis..." << std::endl;
        auto future_result = tts_engine_->synthesizeAsync("This is an asynchronous synthesis test.");
        ...
        if (async_result.success) {
            ...
            std::cout << "❌ Asynchronous synthesis failed: " << async_result.errorMessage << std::endl;
        }
        ...
        tts_engine_->synthesizeWithCallback(
            ...
            [&callback_completed](const tts::SynthesisResult& result) {
                if (result.success) {
                    ...
                    std::cout << "❌ Callback synthesis failed: " << result.errorMessage << std::endl;
                }
        ...
        while (!callback_completed && 
               std::chrono::steady_clock::now() - callback_start < std::chrono::seconds(5)) {
    ...
    void testPipelineIntegration() {
        ...
        uint32_t utterance_id = utterance_manager_->createUtterance("tts_example_session");
        if (utterance_id == 0) {
        ...
        std::cout << "✅ Language configuration set: en → es (voice: es_female_1)" << std::endl;
        
        // Add mock audio data
        std::vector<float> audio_data(16000 * 2, 0.1f); // 2 seconds of audio
        utterance_manager_->addAudioData(utterance_id, audio_data);
        std::cout << "✅ Audio data added: " << audio_data.size() << " samples" << std::endl;
        ...
        while (std::chrono::steady_clock::now() - start_time < timeout) {
            ...
            if (current_state != last_state) {
                std::cout << "📊 Pipeline state: " << stateToString(current_state) << std::endl;
        ...
        if (final_utterance && final_utterance->state == core::UtteranceState::COMPLETE) {
            ...
            std::cout << "   Synthesized audio: " << final_utterance->synthesized_audio.size() << " bytes" << std::endl;
            std::cout << "   Voice used: " << final_utterance->voice_id << std::endl;
            
            // Save final audio
            if (!final_utterance->synthesized_audio.empty()) {
                saveAudioToFile(final_utterance->synthesized_audio, "pipeline_result.wav");
            }
        } else {
    ...
    void demonstrateAdvancedFeatures() {
        ...
        std::string test_text = "This text will be synthesized with different parameters.";
        ...
        for (float speed : speeds) {
            ...
            if (result.success) {
                ...
                std::cout << "  ❌ Speed " << speed << "x failed: " << result.errorMessage << std::endl;
            }
        }
        ...
        for (float volume : volumes) {
            ...
            auto result = tts_engine_->synthesize(test_text);
            if (result.success) {
                std::cout << "  ✅ Volume " << (volume * 100) << "% successful" << std::endl;
            } else {
                std::cout << "  ❌ Volume " << (volume * 100) << "% failed: " << result.errorMessage << std::endl;
            }
        }
    ...
    void demonstrateErrorHandling() {
        ...
        if (!empty_result.success) {
            std::cout << "✅ Empty text properly rejected: " << empty_result.errorMessage << std::endl;
        } else {
        ...
        if (long_result.success) {
            ...
            std::cout << "⚠️  Long text failed: " << long_result.errorMessage << std::endl;
        }
        
        // Test invalid voice ID
        std::cout << "🧪 Testing invalid voice ID handling..." << std::endl;
        auto invalid_voice_result = tts_engine_->synthesize("Test text", "invalid_voice_12345");
        if (!invalid_voice_result.success) {
            std::cout << "✅ Invalid voice ID properly rejected: " << invalid_voice_result.errorMessage << std::endl;
        } else {
            std::cout << "⚠️  Invalid voice ID was handled (fell back to default)" << std::endl;
        }
        
        // Test engine cleanup and reinitialization
        std::cout << "🧪 Testing engine cleanup and reinitialization..." << std::endl;
        tts_engine_->cleanup();
        ...
        if (!after_cleanup_result.success) {
            ...
            std::cout << "⚠️  Synthesis worked after cleanup (unexpected)" << std::endl;
        }
        
        // Reinitialize
        if (tts_engine_->initialize("models/tts/coqui")) {
            std::cout << "✅ Engine reinitialized successfully" << std::endl;
        } else {
            std::cout << "⚠️  Engine reinitialization failed (using mock)" << std::endl;
        }
    }
    ...
    void setupCompletePipeline() {
        ...
        std::cout << "✅ Complete pipeline configured (STT → MT → TTS)" << std::endl;
    }
    
    void saveAudioToFile(const std::vector<uint8_t>& audio_data, const std::string& filename) {
        try {
            std::ofstream file(filename, std::ios::binary);
            if (file.is_open()) {
                file.write(reinterpret_cast<const char*>(audio_data.data()), audio_data.size());
                file.close();
                std::cout << "💾 Audio saved to: " << filename << " (" << audio_data.size() << " bytes)" << std::endl;
            } else {
    ...
    std::string stateToString(core::UtteranceState state) {
        switch (state) {
            case core::UtteranceState::LISTENING: return "LISTENING";
            case core::UtteranceState::TRANSCRIBING: return "TRANSCRIBING";
            case core::UtteranceState::TRANSLATING: return "TRANSLATING";
            case core::UtteranceState::SYNTHESIZING: return "SYNTHESIZING";
            case core::UtteranceState::COMPLETE: return "COMPLETE";
            case core::UtteranceState::ERROR: return "ERROR";
