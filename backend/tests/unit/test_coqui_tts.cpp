...
void testSynthesis() {
  ...
  std::cout << "Synthesized " << testText.length() << " characters into "
            << result.audioData.size() << " bytes of audio (" << result.duration
            << "s)" << std::endl;
...
void testMultiLanguage() {
  ...
  for (const auto &[lang, text] : testCases) {
    ...
    if (!voices.empty()) {
      ...
      assert(result.success);
      assert(!result.audioData.empty());
      std::cout << "Successfully synthesized " << lang << ": \"" << text << "\""
                << std::endl;
    }
  }
...
int main() {
  std::cout << "Running Piper TTS tests..." << std::endl;
