...
StreamingAudioBuffer::StreamingAudioBuffer(size_t maxChunks,
                                           size_t chunkSizeHint)
    : maxChunks_(maxChunks), chunkSizeHint_(chunkSizeHint),
      nextSequenceNumber_(0) {}

StreamingAudioBuffer::~StreamingAudioBuffer() { clear(); }
...
bool StreamingAudioBuffer::addChunk(const AudioChunk &chunk) {
  ...
  if (buffer_.size() >= maxChunks_) {
    ...
    speechrnt::utils::Logger::warn(
        "Audio buffer overflow, dropping oldest chunk");
  }
...
size_t StreamingAudioBuffer::getChunks(std::vector<AudioChunk> &chunks,
                                       size_t maxChunks) {
  std::lock_guard<std::mutex> lock(mutex_);
...
float StreamingAudioBuffer::getUtilization() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return static_cast<float>(buffer_.size()) / static_cast<float>(maxChunks_) *
         100.0f;
}

StreamingOptimizer::StreamingOptimizer()
    : sampleRate_(16000), channels_(1), targetLatencyMs_(50),
      adaptiveChunking_(true), chunkOverlap_(0), currentChunkSize_(1024),
      minChunkSize_(256), maxChunkSize_(4096), totalChunksProcessed_(0),
      totalSamplesProcessed_(0), averageLatencyMs_(0.0),
      averageThroughput_(0.0),
      lastStatsUpdate_(std::chrono::steady_clock::now()) {}

StreamingOptimizer::~StreamingOptimizer() {}

bool StreamingOptimizer::initialize(int sampleRate, int channels,
                                    int targetLatencyMs) {
  ...
  minChunkSize_ =
      static_cast<size_t>(sampleRate * channels * 0.010); // 10ms minimum
  maxChunkSize_ =
      static_cast<size_t>(sampleRate * channels * 0.100); // 100ms maximum
  currentChunkSize_ =
      static_cast<size_t>(sampleRate * channels * targetLatencyMs / 1000.0);

  // Clamp to valid range
  currentChunkSize_ =
      std::max(minChunkSize_, std::min(maxChunkSize_, currentChunkSize_));

  speechrnt::utils::Logger::info(
      "StreamingOptimizer initialized: " + std::to_string(sampleRate) + "Hz, " +
...
bool StreamingOptimizer::processStream(const std::vector<float> &audioData,
  ...
  if (!overlapBuffer_.empty()) {
    ...
    workingData.insert(workingData.end(), overlapBuffer_.begin(),
                       overlapBuffer_.end());
  ...
  while (processedSamples < workingData.size()) {
    ...
    if (chunkSize < minChunkSize_ && remainingSamples < workingData.size()) {
      ...
      overlapBuffer_.assign(workingData.begin() + processedSamples,
                            workingData.end());
      break;
    }
    ...
    if (adaptiveChunking_) {
      ...
      if (outputChunks.size() > 1) { // Need at least 2 chunks to estimate
        float estimatedLatency =
            static_cast<float>(processingTime / outputChunks.size());
        currentChunkSize_ = optimizeChunkSize(
            estimatedLatency, static_cast<float>(targetLatencyMs_));
      }
    }
  }
...
size_t StreamingOptimizer::optimizeChunkSize(float currentLatencyMs,
  ...
  size_t newChunkSize =
      static_cast<size_t>(currentChunkSize_ * adjustmentFactor);
  ...
  if (newChunkSize != currentChunkSize_) {
    speechrnt::utils::Logger::debug(
        "Adjusted chunk size from " + std::to_string(currentChunkSize_) +
...
void StreamingOptimizer::setAdaptiveChunking(bool enabled) {
  ...
  speechrnt::utils::Logger::info("Adaptive chunking " +
...
void StreamingOptimizer::setChunkOverlap(size_t overlapSamples) {
  ...
  speechrnt::utils::Logger::info("Chunk overlap set to " +
                                 std::to_string(overlapSamples) + " samples");
}
...
std::map<std::string, double> StreamingOptimizer::getStreamingStats() const {
  ...
  stats["total_samples_processed"] =
      static_cast<double>(totalSamplesProcessed_.load());
...
size_t StreamingOptimizer::getRecommendedBufferSize() const {
  ...
  size_t bufferSize =
      samplesPerMs * targetLatencyMs_ * 2; // 2x target latency for safety
...
void StreamingOptimizer::updateStats(size_t samplesProcessed,
                                     double latencyMs) {
  std::lock_guard<std::mutex> lock(statsMutex_);
  ...
  averageLatencyMs_ =
      alpha * latencyMs + (1.0 - alpha) * averageLatencyMs_.load();

  if (latencyMs > 0) {
    double throughput = samplesProcessed / (latencyMs / 1000.0);
    averageThroughput_ =
        alpha * throughput + (1.0 - alpha) * averageThroughput_.load();
  }

  lastStatsUpdate_ = std::chrono::steady_clock::now();
}

bool StreamingOptimizer::validateAudioData(
    const std::vector<float> &audioData) const {
  if (audioData.empty()) {
    speechrnt::utils::Logger::warn(
        "Empty audio data provided to streaming optimizer");
    return false;
  }
  ...
  for (float sample : audioData) {
    if (!std::isfinite(sample)) {
      speechrnt::utils::Logger::error(
          "Invalid audio sample detected (NaN or Inf)");
      return false;
    }
  }
...
WebSocketOptimizer::WebSocketOptimizer()
    : maxMessageSize_(65536), compressionEnabled_(true),
      totalMessagesOptimized_(0), totalBytesTransmitted_(0),
      totalBytesCompressed_(0) {}

WebSocketOptimizer::~WebSocketOptimizer() {}

bool WebSocketOptimizer::initialize(size_t maxMessageSize,
                                    bool compressionEnabled) {
  maxMessageSize_ = maxMessageSize;
  compressionEnabled_ = compressionEnabled;

  speechrnt::utils::Logger::info(
      "WebSocketOptimizer initialized: max message size " +
...
bool WebSocketOptimizer::optimizeForTransmission(
...
bool WebSocketOptimizer::batchChunks(
  ...
  for (const auto &chunk : chunks) {
    ...
    currentBatch.insert(currentBatch.end(), serializedChunk.begin(),
                        serializedChunk.end());
  }
...
void WebSocketOptimizer::setCompressionEnabled(bool enabled) {
  ...
  speechrnt::utils::Logger::info("WebSocket compression " +
                                 std::string(enabled ? "enabled" : "disabled"));
}

void WebSocketOptimizer::setMaxMessageSize(size_t maxSize) {
  maxMessageSize_ = maxSize;
  speechrnt::utils::Logger::info("WebSocket max message size set to " +
                                 std::to_string(maxSize) + " bytes");
}

std::map<std::string, double> WebSocketOptimizer::getTransmissionStats() const {
  std::map<std::string, double> stats;
  stats["total_messages_optimized"] =
      static_cast<double>(totalMessagesOptimized_.load());
  stats["total_bytes_transmitted"] =
      static_cast<double>(totalBytesTransmitted_.load());
  stats["total_bytes_compressed"] =
      static_cast<double>(totalBytesCompressed_.load());
  stats["compression_ratio"] =
      totalBytesTransmitted_.load() > 0
          ? static_cast<double>(totalBytesCompressed_.load()) /
                totalBytesTransmitted_.load()
          : 0.0;
...
std::vector<uint8_t>
WebSocketOptimizer::compressData(const std::vector<uint8_t> &data) const {
  // Simple compression placeholder - in production, use a real compression
  // library For now, just return the original data
  totalBytesCompressed_ += data.size();
  return data;
}

std::vector<uint8_t>
WebSocketOptimizer::serializeAudioChunk(const AudioChunk &chunk) const {
  ...
  serialized.insert(
      serialized.end(), reinterpret_cast<const uint8_t *>(&seqNum),
  ...
  serialized.insert(
      serialized.end(), reinterpret_cast<const uint8_t *>(&dataSize),
      reinterpret_cast<const uint8_t *>(&dataSize) + sizeof(uint32_t));

  // Add audio data
  serialized.insert(
      serialized.end(), reinterpret_cast<const uint8_t *>(chunk.data.data()),
      reinterpret_cast<const uint8_t *>(chunk.data.data()) + dataSize);

  return serialized;
}

bool WebSocketOptimizer::shouldBatchChunks(
    const std::vector<AudioChunk> &chunks) const {
  if (chunks.empty()) {
    return false;
  }
