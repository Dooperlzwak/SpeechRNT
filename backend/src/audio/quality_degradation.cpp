...
QualityDegradationManager::QualityDegradationManager()
    : autoAdjustment_(true)
    , adjustmentAggressiveness_(0.5f)
    , targetQualityLevel_(AudioQualityLevel::MEDIUM)
    , totalQualityChanges_(0)
    , qualityDegradations_(0)
    , qualityImprovements_(0)
    , networkBasedChanges_(0)
...
bool QualityDegradationManager::initialize(std::shared_ptr<NetworkMonitor> networkMonitor) {
...
bool QualityDegradationManager::applyNetworkBasedDegradation(NetworkQuality networkQuality,
                                                           const NetworkMetrics& metrics) {
    if (!autoAdjustment_) {
        return false;
    }
    
    AudioQualityParams newParams = calculateNetworkOptimizedParams(networkQuality, metrics);
    ...
    if (shouldApplyDegradation(newParams)) {
        ...
        if (getQualityLevelFromParams(newParams) < getQualityLevelFromParams(oldParams)) {
            qualityDegradations_++;
        } else if (getQualityLevelFromParams(newParams) > getQualityLevelFromParams(oldParams)) {
...
bool QualityDegradationManager::applyResourceBasedDegradation(float cpuUsage, float memoryUsage,
                                                            float processingLatency) {
    if (!autoAdjustment_) {
        return false;
    }
    
    AudioQualityParams newParams = calculateResourceOptimizedParams(cpuUsage, memoryUsage, 
                                                                   processingLatency);
...
AudioQualityParams QualityDegradationManager::getRecommendedParams(NetworkQuality networkQuality,
                                                                  float cpuUsage,
    ...
    AudioQualityParams networkParams = calculateNetworkOptimizedParams(networkQuality, dummyMetrics);
    
    // Apply resource-based adjustments
    AudioQualityParams resourceParams = calculateResourceOptimizedParams(cpuUsage, memoryUsage, 100.0f);
    
    // Combine both considerations - take the more conservative (lower quality) option
    AudioQualityParams combinedParams;
    combinedParams.sampleRate = std::min(networkParams.sampleRate, resourceParams.sampleRate);
    combinedParams.bitDepth = std::min(networkParams.bitDepth, resourceParams.bitDepth);
    combinedParams.channels = std::min(networkParams.channels, resourceParams.channels);
    combinedParams.compressionRatio = std::max(networkParams.compressionRatio, 
                                              resourceParams.compressionRatio);
    combinedParams.enableEnhancement = networkParams.enableEnhancement && 
                                      resourceParams.enableEnhancement;
    combinedParams.enableNoiseReduction = networkParams.enableNoiseReduction && 
                                         resourceParams.enableNoiseReduction;
    combinedParams.bufferSizeMs = std::max(networkParams.bufferSizeMs, resourceParams.bufferSizeMs);
...
void QualityDegradationManager::registerQualityChangeCallback(
    std::function<void(const AudioQualityParams&, const AudioQualityParams&)> callback) {
...
std::map<std::string, double> QualityDegradationManager::getDegradationStats() const {
    std::lock_guard<std::mutex> lock(paramsMutex_);
    
    std::map<std::string, double> stats;
    stats["total_quality_changes"] = static_cast<double>(totalQualityChanges_.load());
    ...
    stats["network_based_changes"] = static_cast<double>(networkBasedChanges_.load());
    stats["resource_based_changes"] = static_cast<double>(resourceBasedChanges_.load());
    ...
    stats["current_compression_ratio"] = static_cast<double>(currentParams_.compressionRatio);
    ...
    stats["adjustment_aggressiveness"] = static_cast<double>(adjustmentAggressiveness_);
...
bool QualityDegradationManager::shouldApplyDegradation(const AudioQualityParams& newParams) const {
    ...
    bool significantChange = 
        ...
        (std::abs(newParams.compressionRatio - currentParams_.compressionRatio) > threshold) ||
        ...
        (std::abs(static_cast<float>(newParams.bufferSizeMs - currentParams_.bufferSizeMs)) > 
         currentParams_.bufferSizeMs * threshold);
    
    return significantChange;
}

void QualityDegradationManager::notifyQualityChange(const AudioQualityParams& oldParams,
...
void QualityDegradationManager::recordQualityChange(const AudioQualityParams& params) {
...
AudioQualityParams QualityDegradationManager::getParamsForQualityLevel(AudioQualityLevel level) const {
...
AdaptiveQualityController::AdaptiveQualityController()
    ...
    , updateIntervalMs_(2000)
    , networkBasedControl_(true)
    , resourceBasedControl_(true)
    , cpuThreshold_(0.8f)
    , memoryThreshold_(0.8f)
    , latencyThreshold_(200.0f)
    , currentCpuUsage_(0.0f)
    , currentMemoryUsage_(0.0f)
    , currentProcessingLatency_(0.0f)
    , totalControlCycles_(0)
    , networkAdjustments_(0)
    , resourceAdjustments_(0) {
}
...
bool AdaptiveQualityController::initialize(std::shared_ptr<QualityDegradationManager> degradationManager,
...
void AdaptiveQualityController::setControlThresholds(float cpuThreshold, float memoryThreshold,
...
std::map<std::string, double> AdaptiveQualityController::getControlStats() const {
    std::lock_guard<std::mutex> lock(resourcesMutex_);
    
    std::map<std::string, double> stats;
    stats["total_control_cycles"] = static_cast<double>(totalControlCycles_.load());
    stats["network_adjustments"] = static_cast<double>(networkAdjustments_.load());
    stats["resource_adjustments"] = static_cast<double>(resourceAdjustments_.load());
    ...
    stats["current_processing_latency"] = static_cast<double>(currentProcessingLatency_);
...
void AdaptiveQualityController::performQualityControl() {
    ...
    if (resourceBasedControl_ && !adjustmentMade) {
        ...
        if (shouldAdjustForResources(cpu, memory, latency)) {
            if (degradationManager_->applyResourceBasedDegradation(cpu, memory, latency)) {
...
bool AdaptiveQualityController::shouldAdjustForNetwork(NetworkQuality quality, 
                                                      const NetworkMetrics& metrics) const {
    // Adjust for poor network conditions
    return quality == NetworkQuality::POOR || quality == NetworkQuality::VERY_POOR ||
           metrics.latencyMs > 200.0f || metrics.packetLossRate > 2.0f;
}

bool AdaptiveQualityController::shouldAdjustForResources(float cpu, float memory, 
                                                        float latency) const {
    // Adjust if any resource threshold is exceeded
    return cpu > cpuThreshold_ || memory > memoryThreshold_ || latency > latencyThreshold_;
}
