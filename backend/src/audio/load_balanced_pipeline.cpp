...
PriorityJobQueue::PriorityJobQueue()
    : maxQueueSize_(1000), currentQueueSize_(0), totalJobsSubmitted_(0),
      totalJobsRetrieved_(0), totalJobsCancelled_(0) {}

PriorityJobQueue::~PriorityJobQueue() { clear(); }
...
bool PriorityJobQueue::submitJob(const ProcessingJob &job) {
  ...
  if (currentQueueSize_ >= maxQueueSize_) {
    ...
    if (currentQueueSize_ >= maxQueueSize_) {
      speechrnt::utils::Logger::warn("Job queue full, rejecting job " +
  ...
  speechrnt::utils::Logger::debug(
      "Job " + std::to_string(job.jobId) + " submitted with priority " +
...
bool PriorityJobQueue::getNextJobWithPriority(ProcessingJob &job,
                                              ProcessingPriority minPriority) {
  std::unique_lock<std::mutex> lock(queueMutex_);
...
bool PriorityJobQueue::cancelJob(uint64_t jobId) {
  ...
  for (auto &[priority, queue] : priorityQueues_) {
    ...
    if (found) {
      speechrnt::utils::Logger::debug("Job " + std::to_string(jobId) +
                                      " cancelled");
      return true;
    }
  }
...
std::map<std::string, double> PriorityJobQueue::getQueueStats() const {
  ...
  stats["total_jobs_retrieved"] =
      static_cast<double>(totalJobsRetrieved_.load());
  stats["total_jobs_cancelled"] =
      static_cast<double>(totalJobsCancelled_.load());
  ...
  stats["queue_utilization"] =
      static_cast<double>(currentQueueSize_) / maxQueueSize_;

  // Per-priority statistics
  stats["critical_jobs"] =
      static_cast<double>(getJobCountByPriority(ProcessingPriority::CRITICAL));
  stats["high_jobs"] =
      static_cast<double>(getJobCountByPriority(ProcessingPriority::HIGH));
  stats["normal_jobs"] =
      static_cast<double>(getJobCountByPriority(ProcessingPriority::NORMAL));
  stats["low_jobs"] =
      static_cast<double>(getJobCountByPriority(ProcessingPriority::LOW));
  stats["background_jobs"] = static_cast<double>(
      getJobCountByPriority(ProcessingPriority::BACKGROUND));
...
size_t
PriorityJobQueue::getJobCountByPriority(ProcessingPriority priority) const {
...
void PriorityJobQueue::setMaxQueueSize(size_t maxSize) {
  ...
  speechrnt::utils::Logger::info("Max queue size set to " +
...
ResourceMonitor::ResourceMonitor()
    : updateIntervalMs_(1000), monitoring_(false) {}

ResourceMonitor::~ResourceMonitor() { stopMonitoring(); }
...
bool ResourceMonitor::startMonitoring() {
  ...
  monitoring_ = true;
  monitoringThread_ =
      std::make_unique<std::thread>(&ResourceMonitor::monitoringLoop, this);
...
bool ResourceMonitor::isResourceConstrained(float cpuThreshold,
                                            float memoryThreshold) const {
  std::lock_guard<std::mutex> lock(resourcesMutex_);
...
void ResourceMonitor::registerResourceCallback(
    ...
    try {
...

      {
        ...
        bool significantChange =
            ...
            std::abs(resources.memoryUsage - currentResources_.memoryUsage) >
                0.1f;
      ...
      speechrnt::utils::PerformanceMonitor::getInstance().recordMetric(
          "pipeline.resource.cpu", resources.cpuUsage, "%");
      speechrnt::utils::PerformanceMonitor::getInstance().recordMetric(
          "pipeline.resource.memory", resources.memoryUsage, "%");

    } catch (const std::exception &e) {
      speechrnt::utils::Logger::error("Resource monitoring error: " +
                                      std::string(e.what()));
    }
...
SystemResources ResourceMonitor::measureSystemResources() {
  ...
  resources.resourceConstrained =
      resources.cpuUsage > 0.8f || resources.memoryUsage > 0.8f;
...
void ResourceMonitor::notifyResourceChange(const SystemResources &resources) {
  for (const auto &callback : resourceCallbacks_) {
    try {
      ...
      speechrnt::utils::Logger::error("Resource callback error: " +
                                      std::string(e.what()));
    }
  }
}
...
float ResourceMonitor::getCpuUsage() const {
  ...
  static long lastTotalUser = 0, lastTotalUserLow = 0, lastTotalSys = 0,
              lastTotalIdle = 0;
...
LoadBalancedProcessingPipeline::LoadBalancedProcessingPipeline()
    : pipelineActive_(false), numWorkerThreads_(4), nextJobId_(1),
      gracefulDegradation_(true), cpuThreshold_(0.8f), memoryThreshold_(0.8f) {

  // Set default job type priorities
  jobTypePriorities_[ProcessingJobType::REAL_TIME_STREAM] =
      ProcessingPriority::CRITICAL;
  jobTypePriorities_[ProcessingJobType::INTERACTIVE_REQUEST] =
      ProcessingPriority::HIGH;
  jobTypePriorities_[ProcessingJobType::BATCH_TRANSCRIPTION] =
      ProcessingPriority::LOW;
  jobTypePriorities_[ProcessingJobType::BACKGROUND_TASK] =
      ProcessingPriority::BACKGROUND;
  jobTypePriorities_[ProcessingJobType::SYSTEM_TASK] =
      ProcessingPriority::NORMAL;
}

LoadBalancedProcessingPipeline::~LoadBalancedProcessingPipeline() { stop(); }

bool LoadBalancedProcessingPipeline::initialize(size_t numWorkerThreads,
                                                size_t maxQueueSize) {
  ...
  bool monitorOk =
      resourceMonitor_->initialize(1000); // 1 second update interval
  ...
  if (queueOk && monitorOk) {
    ...
    speechrnt::utils::Logger::info(
        "LoadBalancedProcessingPipeline initialized with " +
        std::to_string(numWorkerThreads) + " worker threads");
    return true;
  } else {
    speechrnt::utils::Logger::error(
        "LoadBalancedProcessingPipeline initialization failed");
    return false;
  }
}
...
bool LoadBalancedProcessingPipeline::start() {
  ...
  for (size_t i = 0; i < numWorkerThreads_; ++i) {
    workerThreads_.push_back(std::make_unique<std::thread>(
        &LoadBalancedProcessingPipeline::workerLoop, this, i));
  }

  speechrnt::utils::Logger::info("Processing pipeline started with " +
                                 std::to_string(numWorkerThreads_) +
                                 " workers");
  return true;
}
...
uint64_t LoadBalancedProcessingPipeline::submitJob(const ProcessingJob &job) {
  ...
  if (jobQueue_->submitJob(jobCopy)) {
    ...
    speechrnt::utils::Logger::debug("Job " + std::to_string(jobCopy.jobId) +
                                    " submitted successfully");
    return jobCopy.jobId;
  } else {
    speechrnt::utils::Logger::error("Failed to submit job " +
                                    std::to_string(jobCopy.jobId));
    return 0;
  }
}

uint64_t LoadBalancedProcessingPipeline::submitRealTimeJob(
    std::function<void()> task, const std::string &description) {
  ProcessingJob job;
  ...
  job.deadline = std::chrono::steady_clock::now() +
                 std::chrono::milliseconds(200); // 200ms deadline
...
uint64_t
LoadBalancedProcessingPipeline::submitBatchJob(std::function<void()> task,
  ...
  job.description = description;
  job.deadline = std::chrono::steady_clock::now() +
                 std::chrono::minutes(5); // 5 minute deadline
...
void LoadBalancedProcessingPipeline::setJobTypePriority(
    ProcessingJobType jobType, ProcessingPriority priority) {
  jobTypePriorities_[jobType] = priority;
  speechrnt::utils::Logger::info(
      "Job type " + std::to_string(static_cast<int>(jobType)) +
...
void LoadBalancedProcessingPipeline::setGracefulDegradation(bool enabled) {
  ...
  speechrnt::utils::Logger::info("Graceful degradation " +
                                 std::string(enabled ? "enabled" : "disabled"));
}

void LoadBalancedProcessingPipeline::setResourceThresholds(
    float cpuThreshold, float memoryThreshold) {
  cpuThreshold_ = cpuThreshold;
  memoryThreshold_ = memoryThreshold;
  speechrnt::utils::Logger::info(
      "Resource thresholds set: CPU=" + std::to_string(cpuThreshold) +
...
bool LoadBalancedProcessingPipeline::isHealthy() const {
  ...
  bool latencyHealthy =
      resources.averageLatency < 1000.0f; // 1 second threshold

  return resourcesHealthy && queueHealthy && latencyHealthy;
}

std::map<std::string, double>
LoadBalancedProcessingPipeline::getPipelineStats() const {
  std::map<std::string, double> stats;
  ...
  stats["processing.total_jobs_processed"] =
      static_cast<double>(processingStats.totalJobsProcessed);
  stats["processing.total_jobs_completed"] =
      static_cast<double>(processingStats.totalJobsCompleted);
  stats["processing.total_jobs_failed"] =
      static_cast<double>(processingStats.totalJobsFailed);
  stats["processing.average_processing_time"] =
      static_cast<double>(processingStats.averageProcessingTime);
  stats["processing.average_queue_time"] =
      static_cast<double>(processingStats.averageQueueTime);
  stats["processing.throughput"] =
      static_cast<double>(processingStats.throughputJobsPerSecond);
  ...
  stats["resources.active_threads"] =
      static_cast<double>(resources.activeThreads);
  stats["resources.average_latency"] =
      static_cast<double>(resources.averageLatency);
  stats["resources.resource_constrained"] =
      resources.resourceConstrained ? 1.0 : 0.0;
...
void LoadBalancedProcessingPipeline::workerLoop(size_t workerId) {
  speechrnt::utils::Logger::info("Worker thread " + std::to_string(workerId) +
                                 " started");
    ...
    try {
      ...
      if (jobQueue_->getNextJob(job)) {
        ...
        if (shouldProcessJob(job)) {
          ...
          speechrnt::utils::Logger::debug(
              "Worker " + std::to_string(workerId) + " completed job " +
              std::to_string(job.jobId) + " in " +
              std::to_string(processingTime) + "ms");
        } else {
          // Job was skipped due to resource constraints
          speechrnt::utils::Logger::debug(
              "Worker " + std::to_string(workerId) + " skipped job " +
              std::to_string(job.jobId) + " due to resource constraints");
        }
...
void LoadBalancedProcessingPipeline::updateStatistics(const ProcessingJob &job,
  ...
  stats_.averageProcessingTime =
      alpha * processingTime + (1.0f - alpha) * stats_.averageProcessingTime;
  stats_.averageQueueTime =
      alpha * queueTime + (1.0f - alpha) * stats_.averageQueueTime;
...
ProcessingPriority LoadBalancedProcessingPipeline::getJobPriority(
    ProcessingJobType jobType) const {
...
bool LoadBalancedProcessingPipeline::shouldProcessJob(
    const ProcessingJob &job) const {
  SystemResources resources = resourceMonitor_->getCurrentResources();
  ...
  if (resources.resourceConstrained &&
      (job.priority == ProcessingPriority::LOW ||
       job.priority == ProcessingPriority::BACKGROUND)) {
    return false;
  }
...
void LoadBalancedProcessingPipeline::performGracefulDegradation() {
  ...
  // This could involve adjusting queue priorities, reducing worker threads,
  // etc.
  speechrnt::utils::Logger::info(
      "Performing graceful degradation due to resource constraints");
}

void LoadBalancedProcessingPipeline::onResourceChange(
...
std::unique_ptr<LoadBalancedProcessingPipeline>
ProcessingPipelineFactory::createRealTimePipeline(size_t numThreads) {
  ...
  pipeline->setJobTypePriority(ProcessingJobType::REAL_TIME_STREAM,
                               ProcessingPriority::CRITICAL);
  pipeline->setJobTypePriority(ProcessingJobType::INTERACTIVE_REQUEST,
                               ProcessingPriority::HIGH);
  pipeline->setJobTypePriority(ProcessingJobType::BATCH_TRANSCRIPTION,
                               ProcessingPriority::BACKGROUND);
...
std::unique_ptr<LoadBalancedProcessingPipeline>
ProcessingPipelineFactory::createBatchPipeline(size_t numThreads) {
  ...
  pipeline->setJobTypePriority(ProcessingJobType::BATCH_TRANSCRIPTION,
                               ProcessingPriority::HIGH);
  pipeline->setJobTypePriority(ProcessingJobType::BACKGROUND_TASK,
                               ProcessingPriority::NORMAL);
  pipeline->setJobTypePriority(ProcessingJobType::REAL_TIME_STREAM,
                               ProcessingPriority::CRITICAL);
...
std::unique_ptr<LoadBalancedProcessingPipeline>
ProcessingPipelineFactory::createHybridPipeline(size_t numThreads) {
...
std::unique_ptr<LoadBalancedProcessingPipeline>
ProcessingPipelineFactory::createCustomPipeline(
