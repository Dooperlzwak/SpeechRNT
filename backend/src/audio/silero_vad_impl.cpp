...
#ifndef SILERO_MODEL_PATH
#define SILERO_MODEL_PATH "/usr/share/speechrnt/models/silero_vad.onnx"
#endif
...
public:
  ...
  bool initialize(const std::string &modelPath) {
    try {
      ...
      speechrnt::utils::Logger::info(
          "Silero-VAD ONNX model loaded successfully");
      speechrnt::utils::Logger::info("Input shape: [" +
                                     std::to_string(inputShape_[0]) + ", " +
                                     std::to_string(inputShape_[1]) + "]");
      speechrnt::utils::Logger::info("Output shape: [" +
                                     std::to_string(outputShape_[0]) + ", " +
                                     std::to_string(outputShape_[1]) + "]");

      return true;

    } catch (const std::exception &e) {
      speechrnt::utils::Logger::error("Failed to load silero-vad model: " +
                                      std::string(e.what()));
      return false;
    }
  }
  ...
  float runInference(const std::vector<float> &audioData, uint32_t sampleRate) {
    ...
    try {
      ...
      // Audio input tensor
      std::vector<int64_t> audioShape = {
          1, static_cast<int64_t>(audioData.size())};
      ...
      Ort::Value srTensor = Ort::Value::CreateTensor<int64_t>(
          memoryInfo, srData.data(), srData.size(), srShape.data(),
          srShape.size());
      ...
      speechrnt::utils::Logger::error("Silero-VAD inference failed: " +
                                      std::string(e.what()));
      return 0.0f;
    }
  }
...
SileroVadImpl::SileroVadImpl()
    : initialized_(false), sampleRate_(16000), currentMode_(VadMode::HYBRID),
      sileroModelLoaded_(false), energyThreshold_(0.01f),
      energyHistorySize_(10) {
...
SileroVadImpl::~SileroVadImpl() { shutdown(); }

bool SileroVadImpl::initialize(uint32_t sampleRate,
  ...
  if (sileroModelLoaded_) {
    ...
    speechrnt::utils::Logger::warn(
        "SileroVadImpl initialized with energy-based fallback only");
...
void SileroVadImpl::setVadMode(VadMode mode) {
  if (mode == VadMode::SILERO && !sileroModelLoaded_) {
    speechrnt::utils::Logger::warn(
        "Cannot set SILERO mode: model not loaded, using HYBRID instead");
  ...
  speechrnt::utils::Logger::info(
      "VAD mode set to: " + std::to_string(static_cast<int>(currentMode_)));
}
...
float SileroVadImpl::processSamples(const std::vector<float> &samples) {
  ...
  try {
    ...
    speechrnt::utils::Logger::error("VAD processing error: " +
                                    std::string(e.what()));
...
bool SileroVadImpl::loadSileroModel(const std::string &modelPath) {
  ...
  try {
    if (onnxSession_->initialize(modelPath)) {
      speechrnt::utils::Logger::info("Silero-VAD model loaded from: " +
                                     modelPath);
      return true;
    }
  } catch (const std::exception &e) {
...
#else
  speechrnt::utils::Logger::warn(
      "ONNX Runtime not available, cannot load silero-vad model");
#endif
  return false;
}
...
float SileroVadImpl::processSileroVad(const std::vector<float> &samples) {
  ...
  try {
    ...
    // Run inference
    float probability =
        onnxSession_->runInference(processedSamples, sampleRate_);

    return probability;

  } catch (const std::exception &e) {
    speechrnt::utils::Logger::error("Silero-VAD processing failed: " +
                                    std::string(e.what()));
    return -1.0f; // Indicate failure
  }
}
...
float SileroVadImpl::processEnergyBasedVad(const std::vector<float> &samples) {
  ...
  float energyScore = (energy > adaptiveThreshold)
                          ? std::min(1.0f, energy / adaptiveThreshold)
                          : 0.0f;

  float spectralScore = std::min(
      1.0f, spectralCentroid / 2000.0f); // Normalize by typical speech centroid
...
float SileroVadImpl::calculateSpectralCentroid(
  ...
  return (magnitudeSum > 0.0) ? static_cast<float>(weightedSum / magnitudeSum)
                              : 0.0f;
}

std::vector<float>
SileroVadImpl::preprocessAudio(const std::vector<float> &samples) {
...
std::vector<float>
SileroVadImpl::resampleIfNeeded(const std::vector<float> &samples,
...
void SileroVadImpl::updateStatistics(bool usedSilero, float confidence,
  ...
  if (stats_.totalProcessedChunks == 1) {
    ...
    stats_.averageProcessingTimeMs =
        (1.0 - alpha) * stats_.averageProcessingTimeMs +
        alpha * processingTimeMs;
    stats_.averageConfidence =
        (1.0 - alpha) * stats_.averageConfidence + alpha * confidence;
  }
}
...
EnergyBasedVAD::EnergyBasedVAD() : EnergyBasedVAD(Config{}) {}
...
float EnergyBasedVAD::detectVoiceActivity(const std::vector<float> &samples) {
  ...
  float spectralFeature =
      config_.useSpectralFeatures ? calculateSpectralFeatures(samples) : 0.0f;

  if (config_.useAdaptiveThreshold) {
    updateAdaptiveThreshold(energy);
  }

  float threshold = config_.useAdaptiveThreshold ? adaptiveThreshold_
                                                 : config_.energyThreshold;

  // Combine energy and spectral features
  float energyScore =
      (energy > threshold) ? std::min(1.0f, energy / threshold) : 0.0f;
  float spectralScore =
      config_.useSpectralFeatures
          ? std::min(1.0f, spectralFeature / config_.spectralThreshold)
          : 0.0f;
...
float EnergyBasedVAD::calculateSpectralFeatures(
    const std::vector<float> &samples) {
