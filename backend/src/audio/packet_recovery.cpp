...
PacketLossDetector::PacketLossDetector()
    : packetTimeoutMs_(1000), maxRetries_(3) {}
...
bool PacketLossDetector::initialize(int timeoutMs, int maxRetries) {
  ...
  speechrnt::utils::Logger::info(
      "PacketLossDetector initialized: " + std::to_string(timeoutMs) +
      "ms timeout, " + std::to_string(maxRetries) + " max retries");

  return true;
}

void PacketLossDetector::registerSentPacket(uint32_t sequenceNumber,
                                            size_t dataSize) {
...
size_t
PacketLossDetector::detectLostPackets(std::vector<uint32_t> &lostPackets) {
  lostPackets.clear();
...
void PacketLossDetector::resetStats() {
  ...
  speechrnt::utils::Logger::info("Packet loss detector statistics reset");
}
...
void PacketLossDetector::cleanupOldPackets() {
  ...
  for (auto it = acknowledgedPackets_.begin();
       it != acknowledgedPackets_.end();) {
...
AudioChunkReorderBuffer::AudioChunkReorderBuffer()
    : maxBufferSize_(50), reorderTimeoutMs_(500), expectedSequenceNumber_(0),
      totalChunksReceived_(0), totalChunksReordered_(0), totalChunksDropped_(0),
      totalSequenceGaps_(0) {}

AudioChunkReorderBuffer::~AudioChunkReorderBuffer() {}

bool AudioChunkReorderBuffer::initialize(size_t maxBufferSize,
...
size_t
AudioChunkReorderBuffer::getOrderedChunks(std::vector<AudioChunk> &chunks,
                                          size_t maxChunks) {
  chunks.clear();
...
size_t
AudioChunkReorderBuffer::flushBufferedChunks(std::vector<AudioChunk> &chunks) {
  chunks.clear();
...
size_t AudioChunkReorderBuffer::detectSequenceGaps(
    std::vector<uint32_t> &missingSequences) const {
...
std::map<std::string, double> AudioChunkReorderBuffer::getReorderStats() const {
  ...
  stats["total_chunks_reordered"] =
      static_cast<double>(totalChunksReordered_.load());
  stats["total_chunks_dropped"] =
      static_cast<double>(totalChunksDropped_.load());
  ...
  stats["expected_sequence_number"] =
      static_cast<double>(expectedSequenceNumber_);
...
PacketRecoverySystem::PacketRecoverySystem()
    : recoveryEnabled_(true), recoveryAggressiveness_(0.5f), nextPacketId_(1),
      totalChunksProcessed_(0), totalRetransmissions_(0),
      totalRecoveredChunks_(0) {}

PacketRecoverySystem::~PacketRecoverySystem() {}

bool PacketRecoverySystem::initialize(
...
bool PacketRecoverySystem::processOutgoingChunk(const AudioChunk &chunk,
                                                uint32_t &packetId) {
  if (!recoveryEnabled_) {
    packetId = 0;
    return true;
  ...
  lossDetector_->registerSentPacket(packetId,
...
bool PacketRecoverySystem::processIncomingChunk(
...
size_t PacketRecoverySystem::getRetransmissionQueue(
    std::vector<AudioChunk> &retransmitChunks) {
...
void PacketRecoverySystem::updateRecoveryParams(float lossRate, float latencyMs,
                                                float jitterMs) {
  if (!recoveryEnabled_) {
    return;
  }
...
std::map<std::string, double> PacketRecoverySystem::getRecoveryStats() const {
  ...
  stats["total_packets_recovered"] =
      static_cast<double>(lossStats.totalPacketsRecovered);
  ...
  stats["total_retransmissions"] =
      static_cast<double>(totalRetransmissions_.load());
  stats["total_recovered_chunks"] =
      static_cast<double>(totalRecoveredChunks_.load());
  stats["recovery_enabled"] = recoveryEnabled_ ? 1.0 : 0.0;
  stats["recovery_aggressiveness"] =
      static_cast<double>(recoveryAggressiveness_);

  // Calculate recovery effectiveness
  if (lossStats.totalPacketsLost > 0) {
    stats["recovery_effectiveness"] =
        static_cast<double>(lossStats.totalPacketsRecovered) /
...
bool PacketRecoverySystem::shouldRetransmit(const AudioChunk &chunk,
  ...
  auto age = std::chrono::duration_cast<std::chrono::milliseconds>(
                 std::chrono::steady_clock::now() - chunk.timestamp)
                 .count();

  return age > (1000 * threshold); // Retransmit if older than threshold
}

void PacketRecoverySystem::adaptRecoveryParameters(float lossRate,
                                                   float latencyMs,
                                                   float jitterMs) {
  // Adjust packet timeout based on latency and jitter
  int newTimeout = static_cast<int>(latencyMs * 2 + jitterMs * 3);
  newTimeout =
      std::max(500, std::min(5000, newTimeout)); // Clamp between 500ms and 5s
  ...
  speechrnt::utils::Logger::debug(
      "Adapted recovery parameters: timeout=" + std::to_string(newTimeout) +
