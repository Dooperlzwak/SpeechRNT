...
bool ExtendedAudioFormat::isValid() const {
  return static_cast<uint32_t>(sampleRate) > 0 && channels > 0 &&
         codec != AudioCodec::UNKNOWN && chunkSize > 0;
}
...
std::string ExtendedAudioFormat::toString() const {
  std::stringstream ss;
  ss << static_cast<uint32_t>(sampleRate) << "Hz, " << channels
     << " channel(s), ";
...
AudioQualityMetrics::AudioQualityMetrics()
    : signalToNoiseRatio(0.0f), totalHarmonicDistortion(0.0f),
      dynamicRange(0.0f), peakAmplitude(0.0f), rmsLevel(0.0f),
      zeroCrossingRate(0.0f), spectralCentroid(0.0f), hasClipping(false),
      hasSilence(false), noiseFloor(-60.0f) {}

bool AudioQualityMetrics::isGoodQuality() const {
  return signalToNoiseRatio > 20.0f && totalHarmonicDistortion < 5.0f &&
         dynamicRange > 30.0f && !hasClipping && rmsLevel > 0.01f;
}
...
std::string AudioQualityMetrics::getQualityDescription() const {
  ...
  if (signalToNoiseRatio < 20.0f)
    issues.push_back("Low SNR");
  if (totalHarmonicDistortion > 5.0f)
    issues.push_back("High distortion");
  if (dynamicRange < 30.0f)
    issues.push_back("Limited dynamic range");
  if (hasClipping)
    issues.push_back("Clipping detected");
  if (hasSilence)
    issues.push_back("Silence detected");
  if (rmsLevel < 0.01f)
    issues.push_back("Very low level");
...
NoiseProfile::NoiseProfile()
    : noiseLevel(-60.0f), speechLevel(-20.0f), noiseFloor(-60.0f),
      hasBackgroundNoise(false), hasImpulseNoise(false),
      hasPeriodicNoise(false) {}

float NoiseProfile::getSNR() const { return speechLevel - noiseLevel; }
...
const std::vector<AudioCodec> AudioFormatValidator::supportedCodecs_ = {
    AudioCodec::PCM_16, AudioCodec::PCM_24, AudioCodec::PCM_32,
    AudioCodec::FLOAT_32};

const std::unordered_map<AudioCodec, size_t>
    AudioFormatValidator::codecBytesPerSample_ = {{AudioCodec::PCM_16, 2},
                                                  {AudioCodec::PCM_24, 3},
                                                  {AudioCodec::PCM_32, 4},
                                                  {AudioCodec::FLOAT_32, 4}};

bool AudioFormatValidator::isFormatSupported(
    const ExtendedAudioFormat &format) {
  // Check sample rate
  auto srIt = std::find(supportedSampleRates_.begin(),
                        supportedSampleRates_.end(), format.sampleRate);
  ...
  // Check codec
  auto codecIt =
      std::find(supportedCodecs_.begin(), supportedCodecs_.end(), format.codec);
...
bool AudioFormatValidator::canConvertFormat(const ExtendedAudioFormat &from,
...
bool AudioFormatValidator::validateAudioData(
    std::string_view data, const ExtendedAudioFormat &format) {
  if (!format.isValid()) {
    return false;
  }
  ...
  if (data.size() % expectedBytes != 0) {
    speechrnt::utils::Logger::warn("Audio data size not aligned to sample size");
    return false;
  }

  size_t sampleCount = data.size() / expectedBytes;
  if (sampleCount % format.channels != 0) {
    speechrnt::utils::Logger::warn("Audio data not aligned to channel count");
    return false;
  }
...
bool AudioFormatValidator::validateSampleRate(uint32_t sampleRate) {
  ...
  return std::find(supportedSampleRates_.begin(), supportedSampleRates_.end(),
                   sr) != supportedSampleRates_.end();
}
...
bool AudioFormatValidator::validateBitDepth(const AudioCodec &codec) {
  return std::find(supportedCodecs_.begin(), supportedCodecs_.end(), codec) !=
         supportedCodecs_.end();
}
...
std::vector<float>
AudioFormatConverter::resample(const std::vector<float> &input,
...
std::vector<float>
AudioFormatConverter::upsample(const std::vector<float> &input,
                               uint32_t factor) {
  std::vector<float> output;
...
std::vector<float>
AudioFormatConverter::downsample(const std::vector<float> &input,
                                 uint32_t factor) {
...
std::vector<float>
AudioFormatConverter::stereoToMono(const std::vector<float> &stereoData) {
  if (stereoData.size() % 2 != 0) {
    speechrnt::utils::Logger::warn("Stereo data size is not even");
    return {};
  }
...
std::vector<float>
AudioFormatConverter::monoToStereo(const std::vector<float> &monoData) {
...
std::vector<float>
AudioFormatConverter::convertChannels(const std::vector<float> &input,
  ...
  speechrnt::utils::Logger::warn(
      "Unsupported channel conversion: " + std::to_string(inputChannels) +
      " to " + std::to_string(outputChannels));
  return input;
}

std::vector<float> AudioFormatConverter::convertToFloat(std::string_view data,
                                                        AudioCodec codec) {
  ...
  case AudioCodec::PCM_24: {
    ...
    for (size_t i = 0; i < sampleCount; ++i) {
      int32_t sample = (byteData[i * 3] << 8) | (byteData[i * 3 + 1] << 16) |
                       (byteData[i * 3 + 2] << 24);
  ...
  default:
    speechrnt::utils::Logger::error("Unsupported audio codec for conversion");
    break;
  }
...
std::vector<int16_t>
AudioFormatConverter::convertToPCM16(const std::vector<float> &samples) {
...
std::vector<int32_t>
AudioFormatConverter::convertToPCM24(const std::vector<float> &samples) {
...
std::vector<int32_t>
AudioFormatConverter::convertToPCM32(const std::vector<float> &samples) {
  std::vector<int32_t> pcm;
  pcm.reserve(samples.size());
...
std::vector<float>
AudioFormatConverter::convertFormat(std::string_view input,
  ...
  if (inputFormat.channels != outputFormat.channels) {
    floatSamples = convertChannels(floatSamples, inputFormat.channels,
  ...
  if (inputFormat.sampleRate != outputFormat.sampleRate) {
    floatSamples =
        resample(floatSamples, static_cast<uint32_t>(inputFormat.sampleRate),
...
float AudioFormatConverter::interpolate(const std::vector<float> &data,
                                        float index) {
  if (data.empty())
    return 0.0f;
...
std::vector<float>
AudioFormatConverter::applyAntiAliasingFilter(const std::vector<float> &input) {
...
AudioQualityMetrics
AudioQualityAssessor::assessQuality(const std::vector<float> &samples,
                                    uint32_t sampleRate) {
...
float AudioQualityAssessor::calculateSNR(const std::vector<float> &samples) {
  ...
  if (noiseRMS < 1e-10f)
    noiseRMS = 1e-10f; // Avoid division by zero

  return 20.0f * std::log10(signalRMS / noiseRMS);
}

float AudioQualityAssessor::calculateTHD(const std::vector<float> &samples,
                                         uint32_t sampleRate) {
...
float AudioQualityAssessor::calculateDynamicRange(
  ...
  if (noiseFloor < 1e-10f)
    noiseFloor = 1e-10f;

  return 20.0f * std::log10(maxAmplitude / noiseFloor);
}

float AudioQualityAssessor::calculateRMSLevel(
    const std::vector<float> &samples) {
  if (samples.empty())
    return 0.0f;
...
float AudioQualityAssessor::calculateZeroCrossingRate(
...
float AudioQualityAssessor::calculateSpectralCentroid(
    const std::vector<float> &samples, uint32_t sampleRate) {
  auto spectrum = computePowerSpectrum(samples);
  ...
  for (size_t i = 0; i < spectrum.size(); ++i) {
    float frequency = static_cast<float>(i * sampleRate) /
                      (2.0f * static_cast<float>(spectrum.size()));
...
bool AudioQualityAssessor::hasClipping(const std::vector<float> &samples,
                                       float threshold) {
  for (float sample : samples) {
...
bool AudioQualityAssessor::hasSilence(const std::vector<float> &samples,
                                      float threshold) {
  ...
  return static_cast<float>(silentSamples) /
             static_cast<float>(samples.size()) >
         0.9f;
}

float AudioQualityAssessor::estimateNoiseFloor(
    const std::vector<float> &samples) {
  if (samples.empty())
    return -60.0f;
...
AudioQualityAssessor::getQualityIssues(const AudioQualityMetrics &metrics) {
  std::vector<std::string> issues;

  if (metrics.signalToNoiseRatio < 20.0f) {
    issues.push_back("Low signal-to-noise ratio (" +
                     std::to_string(metrics.signalToNoiseRatio) + " dB)");
  }

  if (metrics.totalHarmonicDistortion > 5.0f) {
    issues.push_back("High total harmonic distortion (" +
                     std::to_string(metrics.totalHarmonicDistortion) + "%)");
  }

  if (metrics.dynamicRange < 30.0f) {
    issues.push_back("Limited dynamic range (" +
                     std::to_string(metrics.dynamicRange) + " dB)");
  }
...
bool AudioQualityAssessor::requiresPreprocessing(
    const AudioQualityMetrics &metrics) {
  return !metrics.isGoodQuality();
}

std::vector<float>
AudioQualityAssessor::computeFFT(const std::vector<float> &samples) {
  ...
  for (size_t k = 0; k < N / 2; ++k) {
    ...
    for (size_t n = 0; n < N; ++n) {
      float angle =
          -2.0f * M_PI * static_cast<float>(k * n) / static_cast<float>(N);
...
float AudioQualityAssessor::findPeakAmplitude(
    const std::vector<float> &samples) {
  if (samples.empty())
    return 0.0f;
...
std::vector<float>
AudioQualityAssessor::computePowerSpectrum(const std::vector<float> &samples) {
...
NoiseProfile NoiseDetector::analyzeNoise(const std::vector<float> &samples,
                                         uint32_t sampleRate) {
...
bool NoiseDetector::hasBackgroundNoise(const std::vector<float> &samples,
                                       float threshold) {
...
bool NoiseDetector::hasImpulseNoise(const std::vector<float> &samples) {
  ...
  return static_cast<float>(impulseCount) / static_cast<float>(samples.size()) >
         0.01f;
}

bool NoiseDetector::hasPeriodicNoise(const std::vector<float> &samples,
                                     uint32_t sampleRate) {
  return isPeriodicPattern(samples, sampleRate);
}

std::vector<float>
NoiseDetector::computeNoiseSpectrum(const std::vector<float> &samples) {
...
std::vector<float>
NoiseDetector::identifyNoiseFrequencies(const std::vector<float> &samples,
                                        uint32_t sampleRate) {
  auto spectrum = computeNoiseSpectrum(samples);
  return detectSpectralPeaks(spectrum);
}

NoiseDetector::NoiseType
NoiseDetector::classifyNoise(const NoiseProfile &profile) {
  ...
  if (profile.hasBackgroundNoise) {
    ...
    if (!profile.frequencySpectrum.empty()) {
      float spectralFlatness =
          calculateSpectralFlatness(profile.frequencySpectrum);
...
std::string NoiseDetector::noiseTypeToString(NoiseType type) {
  ...
  case NoiseType::PINK_NOISE:
    return "Pink Noise";
  case NoiseType::BROWN_NOISE:
    return "Brown Noise";
  case NoiseType::IMPULSE_NOISE:
    return "Impulse Noise";
  case NoiseType::PERIODIC_NOISE:
    return "Periodic Noise";
  case NoiseType::ENVIRONMENTAL_NOISE:
...
float NoiseDetector::calculateSpectralFlatness(
  ...
  for (float value : spectrum) {
    if (value > 0.0f) {
      geometricMean *=
          std::pow(value, 1.0f / static_cast<float>(spectrum.size()));
...
std::vector<float>
NoiseDetector::detectSpectralPeaks(const std::vector<float> &spectrum) {
...
bool NoiseDetector::isPeriodicPattern(const std::vector<float> &samples,
                                      uint32_t sampleRate) {
  if (samples.size() < sampleRate / 10)
    return false; // Need at least 100ms of data

  // Simple autocorrelation-based periodicity detection
  size_t maxLag = std::min(
      samples.size() / 4, static_cast<size_t>(sampleRate / 50)); // Max 20ms lag
...
std::vector<float>
AudioPreprocessor::normalizeAmplitude(const std::vector<float> &samples,
                                      float targetLevel) {
  if (samples.empty())
    return samples;
...
std::vector<float>
AudioPreprocessor::removeClipping(const std::vector<float> &samples) {
  ...
  for (float &sample : processed) {
    if (std::abs(sample) > 0.95f) {
      ...
      sample = sign *
               (0.95f - 0.05f * std::exp(-(std::abs(sample) - 0.95f) * 10.0f));
    }
  }
...
std::vector<float>
AudioPreprocessor::removeSilence(const std::vector<float> &samples,
                                 float threshold) {
...
std::vector<float>
AudioPreprocessor::applyGainControl(const std::vector<float> &samples,
                                    float gain) {
  std::vector<float> processed;
  processed.reserve(samples.size());
...
std::vector<float>
AudioPreprocessor::applyHighPassFilter(const std::vector<float> &samples,
                                       uint32_t sampleRate, float cutoffHz) {
  auto coefficients = computeFilterCoefficients(
      cutoffHz / static_cast<float>(sampleRate), sampleRate, true);
  return applyFIRFilter(samples, coefficients);
}

std::vector<float>
AudioPreprocessor::applyLowPassFilter(const std::vector<float> &samples,
                                      uint32_t sampleRate, float cutoffHz) {
  auto coefficients = computeFilterCoefficients(
      cutoffHz / static_cast<float>(sampleRate), sampleRate, false);
  return applyFIRFilter(samples, coefficients);
}

std::vector<float>
AudioPreprocessor::applyBandPassFilter(const std::vector<float> &samples,
                                       uint32_t sampleRate, float lowHz,
                                       float highHz) {
...
std::vector<float>
AudioPreprocessor::reduceNoise(const std::vector<float> &samples,
...
std::vector<float> AudioPreprocessor::spectralSubtraction(
    const std::vector<float> &samples,
...
std::vector<float>
AudioPreprocessor::adaptiveNoiseReduction(const std::vector<float> &samples,
                                          uint32_t sampleRate) {
...
std::vector<float>
AudioPreprocessor::enhanceSpeech(const std::vector<float> &samples,
                                 uint32_t sampleRate) {
...
std::vector<float>
AudioPreprocessor::applyCompression(const std::vector<float> &samples,
...
std::vector<float>
AudioPreprocessor::applyDeEsser(const std::vector<float> &samples,
                                uint32_t sampleRate) {
  ...
  auto sibilantBand =
      applyBandPassFilter(samples, sampleRate, lowFreq, highFreq);
...
std::vector<float>
AudioPreprocessor::applyIIRFilter(const std::vector<float> &samples,
...
std::vector<float>
AudioPreprocessor::applyFIRFilter(const std::vector<float> &samples,
...
std::vector<float>
AudioPreprocessor::computeFilterCoefficients(float cutoff, uint32_t sampleRate,
                                             bool highPass) {
...
AudioStreamValidator::AudioStreamValidator(
    const ExtendedAudioFormat &expectedFormat)
    : expectedFormat_(expectedFormat), minSNR_(15.0f), maxTHD_(10.0f),
      maxLatency_(std::chrono::milliseconds(100)), totalChunks_(0),
      droppedChunks_(0), lastChunkTime_(std::chrono::steady_clock::now()) {}
...
bool AudioStreamValidator::validateChunk(std::string_view data) {
  ...
  auto samples =
      AudioFormatConverter::convertToFloat(data, expectedFormat_.codec);

  // Validate quality
  auto metrics = AudioQualityAssessor::assessQuality(
      samples, static_cast<uint32_t>(expectedFormat_.sampleRate));
  ...
  updateHealthMetrics(static_cast<float>(latency.count()),
                      metrics.signalToNoiseRatio);

  return isValid;
}

bool AudioStreamValidator::validateContinuity(
    const std::vector<float> &samples) {
  if (samples.empty())
    return false;
...
bool AudioStreamValidator::validateLatency(
    std::chrono::milliseconds maxLatency) {
  return maxLatency <= maxLatency_;
}

AudioStreamValidator::StreamHealth
AudioStreamValidator::getStreamHealth() const {
  StreamHealth health;
  ...
  float totalLatency =
      std::accumulate(latencyHistory_.begin(), latencyHistory_.end(), 0.0f);
  health.averageLatency =
      totalLatency / static_cast<float>(latencyHistory_.size());

  // Calculate dropout rate
  health.dropoutRate = totalChunks_ > 0 ? static_cast<float>(droppedChunks_) /
                                              static_cast<float>(totalChunks_)
                                        : 1.0f;

  // Calculate quality score
  float totalQuality =
      std::accumulate(qualityHistory_.begin(), qualityHistory_.end(), 0.0f);
  health.qualityScore =
      totalQuality / static_cast<float>(qualityHistory_.size());

  // Determine overall health
  health.isHealthy =
      health.averageLatency < static_cast<float>(maxLatency_.count()) &&
      health.dropoutRate < 0.05f && health.qualityScore >= minSNR_;

  // Identify issues
  if (health.averageLatency >= static_cast<float>(maxLatency_.count())) {
    health.issues.push_back("High latency (" +
                            std::to_string(health.averageLatency) + "ms)");
  }

  if (health.dropoutRate >= 0.05f) {
    health.issues.push_back("High dropout rate (" +
                            std::to_string(health.dropoutRate * 100.0f) + "%)");
  }

  if (health.qualityScore < minSNR_) {
    health.issues.push_back("Poor audio quality (SNR: " +
                            std::to_string(health.qualityScore) + "dB)");
  }
...
void AudioStreamValidator::setExpectedFormat(
    const ExtendedAudioFormat &format) {
...
void AudioStreamValidator::setLatencyThreshold(
    std::chrono::milliseconds maxLatency) {
...
bool AudioStreamValidator::isWithinLatencyBounds(
