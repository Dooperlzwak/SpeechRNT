...
NetworkMonitor::NetworkMonitor()
    : monitoringIntervalMs_(1000), historySize_(60), monitoring_(false),
      currentQuality_(NetworkQuality::GOOD), totalMeasurements_(0),
      qualityChanges_(0) {}

NetworkMonitor::~NetworkMonitor() { stopMonitoring(); }
...
bool NetworkMonitor::initialize(int monitoringIntervalMs, size_t historySize) {
  ...
  speechrnt::utils::Logger::info(
      "NetworkMonitor initialized: " + std::to_string(monitoringIntervalMs) +
      "ms interval, " + std::to_string(historySize) + " history size");
...
bool NetworkMonitor::startMonitoring() {
  ...
  monitoring_ = true;
  monitoringThread_ =
      std::make_unique<std::thread>(&NetworkMonitor::monitoringLoop, this);
...
void NetworkMonitor::registerConditionCallback(
    std::function<void(const NetworkMetrics &, NetworkQuality)> callback) {
  std::lock_guard<std::mutex> lock(metricsMutex_);
...
bool NetworkMonitor::isNetworkStable(float stabilityThreshold) const {
  ...
  float mean = std::accumulate(latencies.begin(), latencies.end(), 0.0f) /
               latencies.size();
  float variance = 0.0f;
...
std::map<std::string, double> NetworkMonitor::getMonitoringStats() const {
  ...
  stats["current_packet_loss_rate"] =
      static_cast<double>(currentMetrics_.packetLossRate);
  stats["current_bandwidth_kbps"] =
      static_cast<double>(currentMetrics_.bandwidthKbps);
  ...
  while (monitoring_.load()) {
    try {
      ...
      speechrnt::utils::PerformanceMonitor::getInstance().recordMetric(
          "network.latency", metrics.latencyMs, "ms");
      speechrnt::utils::PerformanceMonitor::getInstance().recordMetric(
          "network.packet_loss", metrics.packetLossRate, "%");

    } catch (const std::exception &e) {
      speechrnt::utils::Logger::error("Network monitoring error: " +
                                      std::string(e.what()));
    }

    std::this_thread::sleep_for(
        std::chrono::milliseconds(monitoringIntervalMs_));
  }
...
NetworkMetrics NetworkMonitor::measureNetworkConditions() {
  ...
  if (hIcmpFile != INVALID_HANDLE_VALUE) {
    ...
    DWORD dwRetVal = IcmpSendEcho(hIcmpFile, inet_addr("127.0.0.1"), sendData,
                                  sizeof(sendData), NULL, replyBuffer,
                                  sizeof(replyBuffer), 1000);
  ...
  // Simulate packet loss measurement (in production, use actual network
  // statistics)
...
NetworkQuality
NetworkMonitor::classifyNetworkQuality(const NetworkMetrics &metrics) const {
  // Classification based on latency, jitter, and packet loss
  if (metrics.latencyMs < 50.0f && metrics.jitterMs < 5.0f &&
      metrics.packetLossRate < 0.1f) {
    return NetworkQuality::EXCELLENT;
  } else if (metrics.latencyMs < 100.0f && metrics.jitterMs < 10.0f &&
             metrics.packetLossRate < 0.5f) {
    return NetworkQuality::GOOD;
  } else if (metrics.latencyMs < 200.0f && metrics.jitterMs < 20.0f &&
             metrics.packetLossRate < 2.0f) {
    return NetworkQuality::FAIR;
  } else if (metrics.latencyMs < 500.0f && metrics.jitterMs < 50.0f &&
             metrics.packetLossRate < 5.0f) {
...
void NetworkMonitor::notifyConditionChange(const NetworkMetrics &metrics,
                                           NetworkQuality quality) {
  for (const auto &callback : conditionCallbacks_) {
    try {
      callback(metrics, quality);
    } catch (const std::exception &e) {
      speechrnt::utils::Logger::error("Network condition callback error: " +
                                      std::string(e.what()));
    }
  }
}
...
NetworkAwareStreamingAdapter::NetworkAwareStreamingAdapter()
    : adaptiveMode_(true), totalAdaptations_(0), qualityDegradations_(0),
      qualityImprovements_(0) {}

NetworkAwareStreamingAdapter::~NetworkAwareStreamingAdapter() {}

bool NetworkAwareStreamingAdapter::initialize(
  ...
  if (networkMonitor_) {
    ...
    speechrnt::utils::Logger::info(
        "NetworkAwareStreamingAdapter initialized with network monitor");
    return true;
  }

  speechrnt::utils::Logger::error(
      "NetworkAwareStreamingAdapter initialization failed: no network monitor");
  return false;
}

AdaptiveStreamingParams
NetworkAwareStreamingAdapter::getAdaptiveParams() const {
  std::lock_guard<std::mutex> lock(paramsMutex_);
  return currentParams_;
}

void NetworkAwareStreamingAdapter::updateFromFeedback(float successRate,
  ...
  if (successRate < 0.9f) {
    ...
    currentParams_.bufferSizeMs =
        std::min(currentParams_.bufferSizeMs * 1.2f, 500.0f);
    currentParams_.qualityFactor =
        std::max(currentParams_.qualityFactor * 0.9f, 0.3f);
    ...
    currentParams_.bufferSizeMs =
        std::max(currentParams_.bufferSizeMs * 0.9f, 50.0f);
    currentParams_.qualityFactor =
        std::min(currentParams_.qualityFactor * 1.05f, 1.0f);
...
bool NetworkAwareStreamingAdapter::canImproveQuality() const {
  ...
  return quality == NetworkQuality::EXCELLENT ||
         (quality == NetworkQuality::GOOD &&
          networkMonitor_->isNetworkStable());
}

size_t NetworkAwareStreamingAdapter::getRecommendedBufferSize(
    size_t baseBufferMs) const {
  if (!networkMonitor_) {
    return baseBufferMs;
  }
...
size_t NetworkAwareStreamingAdapter::getRecommendedChunkSize(
    size_t baseChunkMs) const {
  if (!networkMonitor_) {
    return baseChunkMs;
  }
...
NetworkAwareStreamingAdapter::getAdaptationStats() const {
  std::lock_guard<std::mutex> lock(paramsMutex_);
  ...
  stats["current_buffer_size_ms"] =
      static_cast<double>(currentParams_.bufferSizeMs);
  stats["current_chunk_size_ms"] =
      static_cast<double>(currentParams_.chunkSizeMs);
  stats["current_quality_factor"] =
      static_cast<double>(currentParams_.qualityFactor);
  stats["current_max_retries"] = static_cast<double>(currentParams_.maxRetries);
  stats["adaptive_mode_enabled"] = adaptiveMode_ ? 1.0 : 0.0;
  stats["adaptation_history_size"] =
      static_cast<double>(adaptationHistory_.size());

  return stats;
}

void NetworkAwareStreamingAdapter::onNetworkConditionChange(
  ...
  if (isAdaptationNeeded(newParams)) {
    ...
    speechrnt::utils::Logger::info(
        "Adapted streaming parameters for network quality change");
  }
}
...
void NetworkAwareStreamingAdapter::recordAdaptation(
    const AdaptiveStreamingParams &params) {
...
bool NetworkAwareStreamingAdapter::isAdaptationNeeded(
    const AdaptiveStreamingParams &newParams) const {
  // Check if parameters have changed significantly
  const float threshold = 0.1f; // 10% change threshold

  return (std::abs(static_cast<float>(newParams.bufferSizeMs -
                                      currentParams_.bufferSizeMs)) /
              currentParams_.bufferSizeMs >
          threshold) ||
         (std::abs(static_cast<float>(newParams.chunkSizeMs -
                                      currentParams_.chunkSizeMs)) /
              currentParams_.chunkSizeMs >
          threshold) ||
         (std::abs(newParams.qualityFactor - currentParams_.qualityFactor) >
          threshold) ||
