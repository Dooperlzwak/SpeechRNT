...
AudioBufferManager::UtteranceBuffer::UtteranceBuffer()
    : startTime(std::chrono::steady_clock::now()),
      lastAccessTime(std::chrono::steady_clock::now()), maxSizeSamples(0),
      writePosition(0), isActive(true), isCircular(true) {}

AudioBufferManager::UtteranceBuffer::UtteranceBuffer(size_t maxSamples,
                                                     bool circular)
    : startTime(std::chrono::steady_clock::now()),
      lastAccessTime(std::chrono::steady_clock::now()),
      maxSizeSamples(maxSamples), writePosition(0), isActive(true),
      isCircular(circular) {
...
bool AudioBufferManager::UtteranceBuffer::addAudioData(
  ...
  if (!isCircular) {
    ...
    audioData.insert(audioData.end(), audio.begin(),
                     audio.begin() + samplesToAdd);
...
std::vector<float> AudioBufferManager::UtteranceBuffer::getRecentAudioData(
  ...
  for (size_t i = 0; i < samplesToGet; ++i) {
    size_t index =
        (writePosition - 1 - i + audioData.size()) % audioData.size();
    result.push_back(audioData[index]);
  }
...
double AudioBufferManager::UtteranceBuffer::getDurationSeconds(
...
AudioBufferManager::BufferStatistics::BufferStatistics()
    : totalUtterances(0), activeUtterances(0), totalMemoryUsageMB(0),
      peakMemoryUsageMB(0), totalAudioSamples(0), droppedSamples(0),
...
AudioBufferManager::AudioBufferManager(const BufferConfig &config)
    : config_(config), peakMemoryUsage_(0), totalDroppedSamples_(0),
      lastCleanupTime_(std::chrono::steady_clock::now()) {

  speechrnt::utils::Logger::info(
      "AudioBufferManager initialized with max " +
...
bool AudioBufferManager::addAudioData(uint32_t utteranceId,
                                      const std::vector<float> &audio) {
  if (audio.empty()) {
    return true;
  }
  ...
  if (it == utteranceBuffers_.end()) {
    ...
    if (!createUtterance(utteranceId)) {
      speechrnt::utils::Logger::warn(
          "Failed to create utterance buffer for ID: " +
  ...
  if (!it->second->isActive) {
    speechrnt::utils::Logger::debug(
        "Attempted to add audio to inactive utterance: " +
        std::to_string(utteranceId));
    totalDroppedSamples_ += audio.size();
    return false;
  }
  ...
  if (!success) {
    totalDroppedSamples_ += audio.size();
    speechrnt::utils::Logger::warn("Audio buffer full for utterance: " +
                                   std::to_string(utteranceId));
  }
...
std::vector<float> AudioBufferManager::getBufferedAudio(uint32_t utteranceId) {
  ...
  auto it = utteranceBuffers_.find(utteranceId);
  if (it == utteranceBuffers_.end()) {
    speechrnt::utils::Logger::debug("No buffer found for utterance: " +
                                    std::to_string(utteranceId));
    return {};
  }
...
std::vector<float> AudioBufferManager::getRecentAudio(uint32_t utteranceId,
                                                      size_t sampleCount) {
  std::lock_guard<std::mutex> lock(bufferMutex_);
...
bool AudioBufferManager::createUtterance(uint32_t utteranceId,
                                         size_t maxSizeMB) {
  std::lock_guard<std::mutex> lock(bufferMutex_);

  // Check if utterance already exists
  if (utteranceBuffers_.find(utteranceId) != utteranceBuffers_.end()) {
    speechrnt::utils::Logger::debug("Utterance already exists: " +
                                    std::to_string(utteranceId));
    return true;
  }
  ...
  if (utteranceBuffers_.size() >= config_.maxUtterances) {
    speechrnt::utils::Logger::warn("Maximum utterance limit reached (" +
                                   std::to_string(config_.maxUtterances) +
                                   "), cleaning up old buffers");

    // Force cleanup of oldest utterances
    auto oldestUtterances = findOldestUtterances(utteranceBuffers_.size() -
                                                 config_.maxUtterances + 1);
  ...
  auto buffer = std::make_unique<UtteranceBuffer>(maxSamples,
                                                  config_.enableCircularBuffer);
  utteranceBuffers_[utteranceId] = std::move(buffer);

  speechrnt::utils::Logger::debug(
      "Created utterance buffer for ID: " + std::to_string(utteranceId) +
...
void AudioBufferManager::finalizeBuffer(uint32_t utteranceId) {
  ...
  if (it != utteranceBuffers_.end()) {
    ...
    speechrnt::utils::Logger::debug("Finalized utterance buffer: " +
                                    std::to_string(utteranceId));
    updateStatistics();
  }
}
...
void AudioBufferManager::setUtteranceActive(uint32_t utteranceId, bool active) {
  ...
  if (it != utteranceBuffers_.end()) {
    ...
    speechrnt::utils::Logger::debug("Set utterance " +
                                    std::to_string(utteranceId) +
...
void AudioBufferManager::cleanupOldBuffers() {
  ...
  for (const auto &pair : utteranceBuffers_) {
    auto timeSinceAccess =
        std::chrono::duration_cast<std::chrono::milliseconds>(
            now - pair.second->lastAccessTime)
            .count();
  ...
  for (uint32_t utteranceId : toRemove) {
    removeUtteranceInternal(utteranceId);
    speechrnt::utils::Logger::debug("Cleaned up old utterance buffer: " +
                                    std::to_string(utteranceId));
  }

  if (!toRemove.empty()) {
    updateStatistics();
    speechrnt::utils::Logger::info("Cleaned up " +
                                   std::to_string(toRemove.size()) +
                                   " old utterance buffers");
  }
}
...
void AudioBufferManager::forceCleanup() {
  ...
  updateStatistics();
  speechrnt::utils::Logger::info("Force cleanup removed " +
...
AudioBufferManager::BufferStatistics AudioBufferManager::getStatistics() const {
  ...
  if (stats.totalUtterances > 0) {
    size_t totalCapacity =
        stats.totalUtterances * calculateMaxSamples(config_.maxBufferSizeMB);
    stats.averageBufferUtilization =
        static_cast<double>(stats.totalAudioSamples) / totalCapacity;
  }
...
std::string AudioBufferManager::getHealthStatus() const {
  ...
  oss << "  Active Utterances: " << stats.activeUtterances << "/"
      << config_.maxUtterances << "\n";
  oss << "  Memory Usage: " << stats.totalMemoryUsageMB
      << "MB (Peak: " << stats.peakMemoryUsageMB << "MB)\n";
...
std::vector<uint32_t>
AudioBufferManager::findOldestUtterances(size_t count) const {
...
void AudioBufferManager::removeUtteranceInternal(uint32_t utteranceId) {
  ...
  if (it != utteranceBuffers_.end()) {
    ...
    speechrnt::utils::Logger::debug("Removed utterance buffer: " +
                                    std::to_string(utteranceId));
  }
}
