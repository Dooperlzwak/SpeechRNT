...
AudioBuffer::AudioBuffer(size_t maxSizeBytes)
    : maxSizeBytes_(maxSizeBytes), currentSizeBytes_(0),
...
bool AudioBuffer::addChunk(const AudioChunk &chunk) {
  ...
  if (currentSizeBytes_ + chunkSize > maxSizeBytes_) {
    ...
    if (currentSizeBytes_ + chunkSize > maxSizeBytes_) {
      speechrnt::utils::Logger::warn("AudioBuffer: Chunk too large, dropping");
      return false;
    }
  }
...
std::vector<AudioChunk> AudioBuffer::getChunks(size_t maxChunks) {
  ...
  size_t count =
      (maxChunks == 0) ? chunks_.size() : std::min(maxChunks, chunks_.size());
...
double AudioBuffer::getDurationSeconds() const {
  ...
  auto duration =
      std::chrono::duration_cast<std::chrono::microseconds>(newest - oldest);
...
AudioProcessor::AudioProcessor(const AudioFormat &format)
    : format_(format), totalBytesProcessed_(0), totalChunksProcessed_(0),
      nextSequenceNumber_(0) {
...
bool AudioProcessor::validateFormat(const AudioFormat &format) const {
  ...
  if (format.sampleRate != 16000) {
    speechrnt::utils::Logger::error(
        "Invalid sample rate: " + std::to_string(format.sampleRate) +
        " (expected 16000)");
    return false;
  }
  ...
  if (format.channels != 1) {
    speechrnt::utils::Logger::error(
        "Invalid channel count: " + std::to_string(format.channels) +
        " (expected 1)");
    return false;
  }
  ...
  if (format.bitsPerSample != 16) {
    speechrnt::utils::Logger::error(
        "Invalid bits per sample: " + std::to_string(format.bitsPerSample) +
        " (expected 16)");
    return false;
  }
  ...
  if (format.chunkSize == 0 || format.chunkSize > 8192) {
    speechrnt::utils::Logger::error(
        "Invalid chunk size: " + std::to_string(format.chunkSize) +
        " (expected 1-8192)");
    return false;
  }
...
bool AudioProcessor::validatePCMData(std::string_view data) const {
  ...
  if (data.size() % 2 != 0) {
    speechrnt::utils::Logger::warn(
        "PCM data size is not even (16-bit samples expected)");
    return false;
  }
  ...
  if (data.size() != expectedBytes && data.size() > 0) {
    speechrnt::utils::Logger::debug(
        "PCM data size (" + std::to_string(data.size()) +
...
std::vector<float>
AudioProcessor::convertPCMToFloat(std::string_view pcmData) const {
...
std::vector<int16_t>
AudioProcessor::convertFloatToPCM(const std::vector<float> &samples) const {
...
AudioProcessor::processStreamingData(std::string_view data) {
...
AudioIngestionManager::AudioIngestionManager(const std::string &sessionId)
  ...
  speechrnt::utils::Logger::info("AudioIngestionManager created for session: " +
                                 sessionId);
}
...
bool AudioIngestionManager::ingestAudioData(std::string_view data) {
  ...
  try {
    ...
    speechrnt::utils::Logger::error(
        "AudioIngestionManager: Processing error for session " + sessionId_ +
        ": " + e.what());
...
AudioIngestionManager::Statistics AudioIngestionManager::getStatistics() const {
  ...
  result.bufferUtilization =
      static_cast<double>(audioBuffer_->getBufferSizeBytes()) /
...
void AudioIngestionManager::updateStatistics(size_t bytesProcessed,
...
void AudioIngestionManager::setError(ErrorCode error) {
  ...
  if (error != ErrorCode::NONE) {
    speechrnt::utils::Logger::warn("AudioIngestionManager error for session " +
                                   sessionId_ + ": " + getErrorMessage());
  }
}
