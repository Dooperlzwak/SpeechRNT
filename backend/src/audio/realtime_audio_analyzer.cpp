...
template <typename T> bool CircularBuffer<T>::push(const T &item) {
...
template <typename T> bool CircularBuffer<T>::pop(T &item) {
  std::lock_guard<std::mutex> lock(mutex_);
  if (size_ == 0) {
...
template <typename T> std::vector<T> CircularBuffer<T>::pop(size_t count) {
  std::lock_guard<std::mutex> lock(mutex_);
...
template <typename T> size_t CircularBuffer<T>::size() const { return size_; }
...
std::vector<float> RealTimeFFT::computeFFT(const std::vector<float> &samples) {
  ...
  return std::vector<float>(fftOutput_.begin(),
                            fftOutput_.begin() + fftSize_ * 2);
}

std::vector<float>
RealTimeFFT::computeMagnitudeSpectrum(const std::vector<float> &samples) {
...
std::vector<float>
RealTimeFFT::computePowerSpectrum(const std::vector<float> &samples) {
  auto fftResult = computeFFT(samples);
...
size_t RealTimeFFT::getFrequencyBin(float frequency,
...
std::vector<float>
AudioEffectsProcessor::processEffects(const std::vector<float> &input) {
...
std::vector<float>
AudioEffectsProcessor::applyCompressor(const std::vector<float> &input) {
  std::vector<float> output(input.size());

  float threshold = std::pow(10.0f, config_.compressorThreshold / 20.0f);
  float ratio = config_.compressorRatio;
  float attack = std::exp(
      -1.0f / (config_.compressorAttack * 0.001f * 16000.0f)); // Assuming 16kHz
  float release =
      std::exp(-1.0f / (config_.compressorRelease * 0.001f * 16000.0f));
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    if (inputLevel > compressorEnvelope_) {
      compressorEnvelope_ =
          attack * compressorEnvelope_ + (1.0f - attack) * inputLevel;
    } else {
      compressorEnvelope_ =
          release * compressorEnvelope_ + (1.0f - release) * inputLevel;
    }
...
std::vector<float>
AudioEffectsProcessor::applyNoiseGate(const std::vector<float> &input) {
  std::vector<float> output(input.size());
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    if (inputLevel > noiseGateEnvelope_) {
      noiseGateEnvelope_ =
          attack * noiseGateEnvelope_ + (1.0f - attack) * inputLevel;
    } else {
      noiseGateEnvelope_ =
          release * noiseGateEnvelope_ + (1.0f - release) * inputLevel;
    }
...
std::vector<float>
AudioEffectsProcessor::applyEqualizer(const std::vector<float> &input) {
  // Simple 3-band equalizer implementation
  // This is a placeholder - a real implementation would use proper filter
  // design
  return input; // Pass-through for now
}

std::vector<float>
AudioEffectsProcessor::applyReverb(const std::vector<float> &input) {
  std::vector<float> output(input.size());
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    ...
    // Read delayed signal
    size_t readIndex =
        (i + reverbBuffer_.size() - delayLength) % reverbBuffer_.size();
...
RealTimeAudioAnalyzer::RealTimeAudioAnalyzer(uint32_t sampleRate,
                                             size_t bufferSize)
  ...
  // Initialize buffers
  audioBuffer_ = std::make_unique<CircularBuffer<float>>(bufferSize *
                                                         10); // 10x buffer size
  metricsBuffer_ = std::make_unique<CircularBuffer<RealTimeMetrics>>(
      1000); // Store 1000 metrics
  ...
  effectsProcessor_ =
      std::make_unique<AudioEffectsProcessor>(defaultEffectsConfig);
...
RealTimeAudioAnalyzer::~RealTimeAudioAnalyzer() { shutdown(); }
...
bool RealTimeAudioAnalyzer::initialize() {
  ...
  try {
    ...
    processingThread_ = std::make_unique<std::thread>(
        &RealTimeAudioAnalyzer::processingLoop, this);
...
void RealTimeAudioAnalyzer::processAudioChunk(const float *samples,
                                              size_t count) {
...
std::vector<RealTimeMetrics>
RealTimeAudioAnalyzer::getMetricsHistory(size_t samples) const {
...
void RealTimeAudioAnalyzer::registerSpectralCallback(
...
std::vector<float>
RealTimeAudioAnalyzer::applyRealTimeEffects(const std::vector<float> &audio) {
...
void RealTimeAudioAnalyzer::updateEffectsConfig(
    const AudioEffectsConfig &config) {
...
void RealTimeAudioAnalyzer::setUpdateInterval(
...
void RealTimeAudioAnalyzer::updateLevelMetrics(
  ...
  if (peak > peakHold_) {
    ...
    auto timeSincePeak = std::chrono::duration_cast<std::chrono::milliseconds>(
        now - lastPeakTime_);
...
void RealTimeAudioAnalyzer::updateSpectralAnalysis(
    const std::vector<float> &samples) {
  ...
  currentSpectral_.spectralCentroid =
      calculateSpectralCentroid(magnitudeSpectrum, sampleRate_);
  currentSpectral_.spectralBandwidth = calculateSpectralBandwidth(
      magnitudeSpectrum, currentSpectral_.spectralCentroid, sampleRate_);
  currentSpectral_.spectralRolloff =
      calculateSpectralRolloff(magnitudeSpectrum, sampleRate_);
  currentSpectral_.spectralFlatness =
      calculateSpectralFlatness(magnitudeSpectrum);

  // Calculate spectral flux
  if (!previousSpectrum_.empty()) {
    currentSpectral_.spectralFlux =
        calculateSpectralFlux(magnitudeSpectrum, previousSpectrum_);
  }
  previousSpectrum_ = magnitudeSpectrum;

  // Find dominant frequency
  auto maxIt =
      std::max_element(magnitudeSpectrum.begin(), magnitudeSpectrum.end());
  if (maxIt != magnitudeSpectrum.end()) {
    size_t maxBin = std::distance(magnitudeSpectrum.begin(), maxIt);
    currentSpectral_.dominantFrequency =
        fftProcessor_->getFrequencyBin(maxBin, sampleRate_);
  }

  // Calculate MFCC coefficients (simplified)
  currentSpectral_.mfccCoefficients =
      calculateMFCC(magnitudeSpectrum, sampleRate_);
}

void RealTimeAudioAnalyzer::updateNoiseEstimation(
    const std::vector<float> &samples) {
  if (samples.empty()) {
    return;
  }
...
void RealTimeAudioAnalyzer::updateSpeechDetection(
    const AudioLevelMetrics &levels, const SpectralAnalysis &spectral) {
  ...
  if (previousLevel > 0.1f && currentLevel < 0.01f) {
    ...
    dropout.timestampMs =
        std::chrono::duration_cast<std::chrono::milliseconds>(
            ...
            .count();
    dropout.durationMs =
        static_cast<float>(samples.size()) / sampleRate_ * 1000.0f;
    ...
    detectedDropouts_.erase(std::remove_if(detectedDropouts_.begin(),
                                           detectedDropouts_.end(),
                                           [cutoffTime](const AudioDropout &d) {
                                             return d.timestampMs < cutoffTime;
                                           }),
                            detectedDropouts_.end());
  }
...
void RealTimeAudioAnalyzer::updatePerformanceMetrics(
  ...
  performanceMetrics_.averageProcessingTimeMs =
      0.9f * performanceMetrics_.averageProcessingTimeMs +
      0.1f * processingTimeMs;
...
float RealTimeAudioAnalyzer::calculateSpectralCentroid(
    const std::vector<float> &spectrum, uint32_t sampleRate) {
  if (spectrum.empty())
    return 0.0f;
...
float RealTimeAudioAnalyzer::calculateSpectralBandwidth(
    const std::vector<float> &spectrum, float centroid, uint32_t sampleRate) {
  if (spectrum.empty())
    return 0.0f;
...
float RealTimeAudioAnalyzer::calculateSpectralRolloff(
    const std::vector<float> &spectrum, uint32_t sampleRate,
...
float RealTimeAudioAnalyzer::calculateSpectralFlatness(
...
float RealTimeAudioAnalyzer::calculateSpectralFlux(
...
std::vector<float>
RealTimeAudioAnalyzer::calculateMFCC(const std::vector<float> &spectrum,
                                     uint32_t sampleRate) {
  // Simplified MFCC calculation - in a real implementation, this would use
  // mel-scale filtering
...
float RealTimeAudioAnalyzer::estimateNoiseLevel(
    const std::vector<float> &samples) {
  if (samples.empty())
    return noiseFloorThreshold_;
...
float RealTimeAudioAnalyzer::calculateSpeechProbability(
    const AudioLevelMetrics &levels, const SpectralAnalysis &spectral) {
  // Simple speech detection heuristic
  float levelScore = levels.currentLevel > silenceThreshold_ ? 1.0f : 0.0f;
  float spectralScore =
      spectral.spectralCentroid > 200.0f && spectral.spectralCentroid < 4000.0f
          ? 1.0f
          : 0.0f;
...
bool RealTimeAudioAnalyzer::detectClipping(const std::vector<float> &samples,
                                           float threshold) {
  for (float sample : samples) {
...
bool RealTimeAudioAnalyzer::detectSilence(const std::vector<float> &samples,
                                          float threshold) {
  float rms = calculateRMS(samples);
  return rms < threshold;
}

void RealTimeAudioAnalyzer::notifyMetricsCallbacks(
...
void RealTimeAudioAnalyzer::notifyLevelsCallbacks(
    const AudioLevelMetrics &levels) {
  std::lock_guard<std::mutex> lock(callbackMutex_);
...
void RealTimeAudioAnalyzer::notifySpectralCallbacks(
...
std::vector<RealTimeAudioAnalyzer::AudioDropout>
RealTimeAudioAnalyzer::getDetectedDropouts() const {
  std::lock_guard<std::mutex> lock(dropoutMutex_);
...
bool RealTimeAudioAnalyzer::hasRecentDropouts(
...
RealTimeAudioAnalyzer::PerformanceMetrics
RealTimeAudioAnalyzer::getPerformanceMetrics() const {
  std::lock_guard<std::mutex> lock(performanceMutex_);
