...
MTConfig::MTConfig()
    : configVersion_("1.0.0"), lastModified_(std::chrono::system_clock::now()),
      configSource_("default"), environment_("development"),
  ...
  languageDetectionConfig_.fallbackLanguages = {
      {"unknown", "en"}, {"auto", "en"}, {"zh-cn", "zh"}, {"zh-tw", "zh"}};
}
  ...
  try {
    ...
    if (rootObj.find("gpu") != rootObj.end()) {
      ...
      if (gpuObj.find("defaultDeviceId") != gpuObj.end()) {
        gpuConfig_.defaultDeviceId =
            static_cast<int>(gpuObj.at("defaultDeviceId").asNumber());
      }
      if (gpuObj.find("memoryPoolSizeMB") != gpuObj.end()) {
        gpuConfig_.memoryPoolSizeMB =
            static_cast<size_t>(gpuObj.at("memoryPoolSizeMB").asNumber());
      }
      if (gpuObj.find("maxModelMemoryMB") != gpuObj.end()) {
        gpuConfig_.maxModelMemoryMB =
            static_cast<size_t>(gpuObj.at("maxModelMemoryMB").asNumber());
      }
      if (gpuObj.find("memoryReservationRatio") != gpuObj.end()) {
        gpuConfig_.memoryReservationRatio =
            static_cast<float>(gpuObj.at("memoryReservationRatio").asNumber());
      }
      if (gpuObj.find("allowedDeviceIds") != gpuObj.end()) {
        ...
        for (const auto &device : deviceArray) {
          gpuConfig_.allowedDeviceIds.push_back(
              static_cast<int>(device.asNumber()));
        }
      }
    }
    ...
    if (rootObj.find("quality") != rootObj.end()) {
      ...
      if (qualityObj.find("highQualityThreshold") != qualityObj.end()) {
        qualityConfig_.highQualityThreshold = static_cast<float>(
            qualityObj.at("highQualityThreshold").asNumber());
      }
      if (qualityObj.find("mediumQualityThreshold") != qualityObj.end()) {
        qualityConfig_.mediumQualityThreshold = static_cast<float>(
            qualityObj.at("mediumQualityThreshold").asNumber());
      }
      if (qualityObj.find("lowQualityThreshold") != qualityObj.end()) {
        qualityConfig_.lowQualityThreshold =
            static_cast<float>(qualityObj.at("lowQualityThreshold").asNumber());
      }
      if (qualityObj.find("generateAlternatives") != qualityObj.end()) {
        qualityConfig_.generateAlternatives =
            qualityObj.at("generateAlternatives").asBool();
      }
      if (qualityObj.find("maxAlternatives") != qualityObj.end()) {
        qualityConfig_.maxAlternatives =
            static_cast<int>(qualityObj.at("maxAlternatives").asNumber());
      }
      if (qualityObj.find("enableFallbackTranslation") != qualityObj.end()) {
        qualityConfig_.enableFallbackTranslation =
            qualityObj.at("enableFallbackTranslation").asBool();
      }
    }
    ...
    if (rootObj.find("caching") != rootObj.end()) {
      ...
      if (cachingObj.find("maxCacheSize") != cachingObj.end()) {
        cachingConfig_.maxCacheSize =
            static_cast<size_t>(cachingObj.at("maxCacheSize").asNumber());
      }
      if (cachingObj.find("cacheExpirationTimeMinutes") != cachingObj.end()) {
        cachingConfig_.cacheExpirationTime =
            std::chrono::minutes(static_cast<int>(
                cachingObj.at("cacheExpirationTimeMinutes").asNumber()));
      }
      ...
      if (cachingObj.find("cacheDirectory") != cachingObj.end()) {
        cachingConfig_.cacheDirectory =
            cachingObj.at("cacheDirectory").asString();
      }
    }
    ...
    if (rootObj.find("batch") != rootObj.end()) {
      ...
      if (batchObj.find("maxBatchSize") != batchObj.end()) {
        batchConfig_.maxBatchSize =
            static_cast<size_t>(batchObj.at("maxBatchSize").asNumber());
      }
      ...
      if (batchObj.find("enableBatchOptimization") != batchObj.end()) {
        batchConfig_.enableBatchOptimization =
            batchObj.at("enableBatchOptimization").asBool();
      }
      if (batchObj.find("optimalBatchSize") != batchObj.end()) {
        batchConfig_.optimalBatchSize =
            static_cast<size_t>(batchObj.at("optimalBatchSize").asNumber());
      }
    }
    ...
    if (rootObj.find("streaming") != rootObj.end()) {
      ...
      if (streamingObj.find("sessionTimeoutMinutes") != streamingObj.end()) {
        streamingConfig_.sessionTimeout = std::chrono::minutes(static_cast<int>(
            streamingObj.at("sessionTimeoutMinutes").asNumber()));
      }
      if (streamingObj.find("maxConcurrentSessions") != streamingObj.end()) {
        streamingConfig_.maxConcurrentSessions = static_cast<size_t>(
            streamingObj.at("maxConcurrentSessions").asNumber());
      }
      if (streamingObj.find("maxContextLength") != streamingObj.end()) {
        streamingConfig_.maxContextLength =
            static_cast<size_t>(streamingObj.at("maxContextLength").asNumber());
      }
      if (streamingObj.find("enableContextPreservation") !=
          streamingObj.end()) {
        streamingConfig_.enableContextPreservation =
            streamingObj.at("enableContextPreservation").asBool();
      }
    }
    ...
    if (rootObj.find("errorHandling") != rootObj.end()) {
      ...
      if (errorObj.find("maxRetryAttempts") != errorObj.end()) {
        errorHandlingConfig_.maxRetryAttempts =
            static_cast<int>(errorObj.at("maxRetryAttempts").asNumber());
      }
      ...
      if (errorObj.find("retryBackoffMultiplier") != errorObj.end()) {
        errorHandlingConfig_.retryBackoffMultiplier = static_cast<float>(
            errorObj.at("retryBackoffMultiplier").asNumber());
      }
      ...
      if (errorObj.find("enableDegradedMode") != errorObj.end()) {
        errorHandlingConfig_.enableDegradedMode =
            errorObj.at("enableDegradedMode").asBool();
      }
      if (errorObj.find("enableFallbackTranslation") != errorObj.end()) {
        errorHandlingConfig_.enableFallbackTranslation =
            errorObj.at("enableFallbackTranslation").asBool();
      }
    }
    ...
    if (rootObj.find("performance") != rootObj.end()) {
      ...
      if (perfObj.find("metricsCollectionIntervalSeconds") != perfObj.end()) {
        performanceConfig_.metricsCollectionInterval =
            std::chrono::seconds(static_cast<int>(
                perfObj.at("metricsCollectionIntervalSeconds").asNumber()));
      }
      if (perfObj.find("enableLatencyTracking") != perfObj.end()) {
        performanceConfig_.enableLatencyTracking =
            perfObj.at("enableLatencyTracking").asBool();
      }
      if (perfObj.find("enableThroughputTracking") != perfObj.end()) {
        performanceConfig_.enableThroughputTracking =
            perfObj.at("enableThroughputTracking").asBool();
      }
      if (perfObj.find("enableResourceUsageTracking") != perfObj.end()) {
        performanceConfig_.enableResourceUsageTracking =
            perfObj.at("enableResourceUsageTracking").asBool();
      }
      if (perfObj.find("maxMetricsHistorySize") != perfObj.end()) {
        performanceConfig_.maxMetricsHistorySize =
            static_cast<size_t>(perfObj.at("maxMetricsHistorySize").asNumber());
      }
    }
    ...
    if (rootObj.find("languageDetection") != rootObj.end()) {
      ...
      if (langDetObj.find("confidenceThreshold") != langDetObj.end()) {
        languageDetectionConfig_.confidenceThreshold =
            static_cast<float>(langDetObj.at("confidenceThreshold").asNumber());
      }
      if (langDetObj.find("detectionMethod") != langDetObj.end()) {
        languageDetectionConfig_.detectionMethod =
            langDetObj.at("detectionMethod").asString();
      }
      if (langDetObj.find("enableHybridDetection") != langDetObj.end()) {
        languageDetectionConfig_.enableHybridDetection =
            langDetObj.at("enableHybridDetection").asBool();
      }
      if (langDetObj.find("hybridWeightText") != langDetObj.end()) {
        languageDetectionConfig_.hybridWeightText =
            static_cast<float>(langDetObj.at("hybridWeightText").asNumber());
      }
      if (langDetObj.find("hybridWeightAudio") != langDetObj.end()) {
        languageDetectionConfig_.hybridWeightAudio =
            static_cast<float>(langDetObj.at("hybridWeightAudio").asNumber());
      }
      ...
      if (langDetObj.find("supportedLanguages") != langDetObj.end()) {
        ...
        for (const auto &lang : langArray) {
          languageDetectionConfig_.supportedLanguages.push_back(
              lang.asString());
        }
      }
      ...
      if (langDetObj.find("fallbackLanguages") != langDetObj.end()) {
        ...
        for (const auto &pair : fallbackObj) {
          languageDetectionConfig_.fallbackLanguages[pair.first] =
    ...
    if (rootObj.find("models") != rootObj.end()) {
      ...
      for (const auto &modelPair : modelsObj) {
        ...
        if (modelObj.find("accuracy") != modelObj.end()) {
          config.accuracy =
              static_cast<float>(modelObj.at("accuracy").asNumber());
        }
        if (modelObj.find("estimatedSizeMB") != modelObj.end()) {
          config.estimatedSizeMB =
              static_cast<size_t>(modelObj.at("estimatedSizeMB").asNumber());
        }
    ...
    // Load model families
    if (rootObj.find("modelFamilies") != rootObj.end()) {
      const auto &familiesObj = rootObj.at("modelFamilies").asObject();

      for (const auto &familyPair : familiesObj) {
        const std::string &familyName = familyPair.first;
        const auto &familyObj = familyPair.second.asObject();

        ModelFamilyConfig config;

        if (familyObj.find("enabled") != familyObj.end()) {
          config.enabled = familyObj.at("enabled").asBool();
        }
        if (familyObj.find("default") != familyObj.end()) {
          config.isDefault = familyObj.at("default").asBool();
        }
        if (familyObj.find("basePath") != familyObj.end()) {
          config.basePath = familyObj.at("basePath").asString();
        }
        if (familyObj.find("description") != familyObj.end()) {
          config.description = familyObj.at("description").asString();
        }

        modelFamilies_[familyName] = config;
      }
    }
...
std::string MTConfig::toJson() const {
  ...
  json << "    \"enabled\": " << (gpuConfig_.enabled ? "true" : "false")
       << ",\n";
  json << "    \"fallbackToCPU\": "
       << (gpuConfig_.fallbackToCPU ? "true" : "false") << ",\n";
  ...
  json << "    \"enabled\": " << (qualityConfig_.enabled ? "true" : "false")
       << ",\n";
  json << "    \"highQualityThreshold\": "
       << qualityConfig_.highQualityThreshold << ",\n";
  json << "    \"mediumQualityThreshold\": "
       << qualityConfig_.mediumQualityThreshold << ",\n";
  json << "    \"lowQualityThreshold\": " << qualityConfig_.lowQualityThreshold
       << ",\n";
  json << "    \"generateAlternatives\": "
       << (qualityConfig_.generateAlternatives ? "true" : "false") << ",\n";
  json << "    \"maxAlternatives\": " << qualityConfig_.maxAlternatives
       << ",\n";
  json << "    \"enableFallbackTranslation\": "
       << (qualityConfig_.enableFallbackTranslation ? "true" : "false") << "\n";
  json << "  },\n";
  ...
  json << "    \"cacheExpirationTimeMinutes\": "
       << cachingConfig_.cacheExpirationTime.count() << ",\n";
  json << "    \"persistToDisk\": "
       << (cachingConfig_.persistToDisk ? "true" : "false") << ",\n";
  ...
  json << "    \"batchTimeoutMs\": " << batchConfig_.batchTimeout.count()
       << ",\n";
  json << "    \"enableBatchOptimization\": "
       << (batchConfig_.enableBatchOptimization ? "true" : "false") << ",\n";
  ...
  json << "    \"sessionTimeoutMinutes\": "
       << streamingConfig_.sessionTimeout.count() << ",\n";
  json << "    \"maxConcurrentSessions\": "
       << streamingConfig_.maxConcurrentSessions << ",\n";
  json << "    \"maxContextLength\": " << streamingConfig_.maxContextLength
       << ",\n";
  json << "    \"enableContextPreservation\": "
       << (streamingConfig_.enableContextPreservation ? "true" : "false")
       << "\n";
  json << "  },\n";
  ...
  json << "  \"errorHandling\": {\n";
  json << "    \"enableRetry\": "
       << (errorHandlingConfig_.enableRetry ? "true" : "false") << ",\n";
  json << "    \"maxRetryAttempts\": " << errorHandlingConfig_.maxRetryAttempts
       << ",\n";
  json << "    \"initialRetryDelayMs\": "
       << errorHandlingConfig_.initialRetryDelay.count() << ",\n";
  json << "    \"retryBackoffMultiplier\": "
       << errorHandlingConfig_.retryBackoffMultiplier << ",\n";
  json << "    \"maxRetryDelayMs\": "
       << errorHandlingConfig_.maxRetryDelay.count() << ",\n";
  json << "    \"translationTimeoutMs\": "
       << errorHandlingConfig_.translationTimeout.count() << ",\n";
  json << "    \"enableDegradedMode\": "
       << (errorHandlingConfig_.enableDegradedMode ? "true" : "false") << ",\n";
  json << "    \"enableFallbackTranslation\": "
       << (errorHandlingConfig_.enableFallbackTranslation ? "true" : "false")
       << "\n";
  json << "  },\n";
  ...
  json << "    \"enabled\": " << (performanceConfig_.enabled ? "true" : "false")
       << ",\n";
  json << "    \"metricsCollectionIntervalSeconds\": "
       << performanceConfig_.metricsCollectionInterval.count() << ",\n";
  json << "    \"enableLatencyTracking\": "
       << (performanceConfig_.enableLatencyTracking ? "true" : "false")
       << ",\n";
  json << "    \"enableThroughputTracking\": "
       << (performanceConfig_.enableThroughputTracking ? "true" : "false")
       << ",\n";
  json << "    \"enableResourceUsageTracking\": "
       << (performanceConfig_.enableResourceUsageTracking ? "true" : "false")
       << ",\n";
  json << "    \"maxMetricsHistorySize\": "
       << performanceConfig_.maxMetricsHistorySize << "\n";
  json << "  },\n";
  ...
  json << "    \"enabled\": "
       << (languageDetectionConfig_.enabled ? "true" : "false") << ",\n";
  json << "    \"confidenceThreshold\": "
       << languageDetectionConfig_.confidenceThreshold << ",\n";
  json << "    \"detectionMethod\": \""
       << languageDetectionConfig_.detectionMethod << "\",\n";
  json << "    \"enableHybridDetection\": "
       << (languageDetectionConfig_.enableHybridDetection ? "true" : "false")
       << ",\n";
  json << "    \"hybridWeightText\": "
       << languageDetectionConfig_.hybridWeightText << ",\n";
  json << "    \"hybridWeightAudio\": "
       << languageDetectionConfig_.hybridWeightAudio << ",\n";
  json << "    \"supportedLanguages\": [";
  for (size_t i = 0; i < languageDetectionConfig_.supportedLanguages.size();
       ++i) {
    if (i > 0)
      json << ", ";
  ...
  json << "  \"modelFamilies\": {\n";
  first = true;
  for (const auto &familyPair : modelFamilies_) {
    if (!first)
      json << ",\n";
    const auto &config = familyPair.second;
    json << "    \"" << familyPair.first << "\": {\n";
    json << "      \"enabled\": " << (config.enabled ? "true" : "false")
         << ",\n";
    json << "      \"default\": " << (config.isDefault ? "true" : "false")
         << ",\n";
    json << "      \"basePath\": \"" << config.basePath << "\",\n";
      ...
      if (section == "gpu") {
        ...
        for (const auto &gpuUpdate : gpuUpdates) {
          if (gpuUpdate.first == "enabled") {
            ...
            gpuConfig_.defaultDeviceId =
                static_cast<int>(gpuUpdate.second.asNumber());
          } else if (gpuUpdate.first == "memoryPoolSizeMB") {
            gpuConfig_.memoryPoolSizeMB =
                static_cast<size_t>(gpuUpdate.second.asNumber());
          } else if (gpuUpdate.first == "maxModelMemoryMB") {
            gpuConfig_.maxModelMemoryMB =
                static_cast<size_t>(gpuUpdate.second.asNumber());
          } else if (gpuUpdate.first == "memoryReservationRatio") {
            gpuConfig_.memoryReservationRatio =
                static_cast<float>(gpuUpdate.second.asNumber());
          }
        }
        ...
        for (const auto &qualityUpdate : qualityUpdates) {
          if (qualityUpdate.first == "enabled") {
            ...
            qualityConfig_.highQualityThreshold =
                static_cast<float>(qualityUpdate.second.asNumber());
            ...
            qualityConfig_.maxAlternatives =
                static_cast<int>(qualityUpdate.second.asNumber());
          } else if (qualityUpdate.first == "enableFallbackTranslation") {
            qualityConfig_.enableFallbackTranslation =
                qualityUpdate.second.asBool();
          }
        }
        ...
        for (const auto &cachingUpdate : cachingUpdates) {
          if (cachingUpdate.first == "enabled") {
            ...
            cachingConfig_.maxCacheSize =
                static_cast<size_t>(cachingUpdate.second.asNumber());
...
std::vector<std::string> MTConfig::getValidationErrors() const {
  ...
  if (gpuConfig_.memoryReservationRatio < 0.0f ||
      gpuConfig_.memoryReservationRatio > 1.0f) {
    errors.push_back("GPU memoryReservationRatio must be between 0.0 and 1.0");
  }

  // Validate quality configuration
  if (qualityConfig_.highQualityThreshold <
      qualityConfig_.mediumQualityThreshold) {
    errors.push_back(
        "Quality highQualityThreshold must be >= mediumQualityThreshold");
  }
  if (qualityConfig_.mediumQualityThreshold <
      qualityConfig_.lowQualityThreshold) {
    errors.push_back(
        "Quality mediumQualityThreshold must be >= lowQualityThreshold");
  }
  ...
  if (languageDetectionConfig_.confidenceThreshold < 0.0f ||
      languageDetectionConfig_.confidenceThreshold > 1.0f) {
    errors.push_back(
        "LanguageDetection confidenceThreshold must be between 0.0 and 1.0");
  }
  if (languageDetectionConfig_.hybridWeightText +
          languageDetectionConfig_.hybridWeightAudio !=
      1.0f) {
...
void MTConfig::setCustomModelPath(const std::string &sourceLang,
...
std::string MTConfig::getModelPath(const std::string &sourceLang,
...
bool MTConfig::hasCustomModelPath(const std::string &sourceLang,
                                  const std::string &targetLang) const {
  std::lock_guard<std::mutex> lock(configMutex_);
  std::string key = getLanguagePairKey(sourceLang, targetLang);
...
void MTConfig::addLanguagePair(const std::string &sourceLang,
                               const std::string &targetLang,
                               const MarianModelConfig &config) {
  std::lock_guard<std::mutex> lock(configMutex_);
  std::string key = getLanguagePairKey(sourceLang, targetLang);
...
void MTConfig::removeLanguagePair(const std::string &sourceLang,
                                  const std::string &targetLang) {
  std::lock_guard<std::mutex> lock(configMutex_);
  std::string key = getLanguagePairKey(sourceLang, targetLang);
...
bool MTConfig::hasLanguagePair(const std::string &sourceLang,
                               const std::string &targetLang) const {
  std::lock_guard<std::mutex> lock(configMutex_);
  std::string key = getLanguagePairKey(sourceLang, targetLang);
...
MarianModelConfig
MTConfig::getModelConfig(const std::string &sourceLang,
                         const std::string &targetLang) const {
  std::lock_guard<std::mutex> lock(configMutex_);
  std::string key = getLanguagePairKey(sourceLang, targetLang);
...
MTConfig::getAvailableLanguagePairs() const {
  std::lock_guard<std::mutex> lock(configMutex_);
...
std::vector<std::string> MTConfig::getSupportedSourceLanguages() const {
  ...
  return std::vector<std::string>(sourceLanguages.begin(),
                                  sourceLanguages.end());
}

std::vector<std::string>
MTConfig::getSupportedTargetLanguages(const std::string &sourceLang) const {
  std::lock_guard<std::mutex> lock(configMutex_);
...
void MTConfig::registerConfigChangeCallback(const std::string &callbackId,
...
std::string MTConfig::getLanguagePairKey(const std::string &sourceLang,
                                         const std::string &targetLang) const {
  return sourceLang + "->" + targetLang;
}

void MTConfig::notifyConfigChange(const std::string &section,
...
std::string
MTConfig::getEnvironmentConfigPath(const std::string &environment) const {
