...
PipelineWebSocketIntegration::PipelineWebSocketIntegration(
    ...
    : pipeline_(pipeline), websocket_server_(websocket_server), active_(false) {
}

PipelineWebSocketIntegration::~PipelineWebSocketIntegration() { shutdown(); }

bool PipelineWebSocketIntegration::initialize() {
  if (active_) {
    speechrnt::utils::Logger::warn(
        "PipelineWebSocketIntegration already initialized");
    return true;
  }

  if (!pipeline_ || !websocket_server_) {
    speechrnt::utils::Logger::error(
        "PipelineWebSocketIntegration initialization failed: null pipeline or "
        "websocket server");
    return false;
  }
  ...
  pipeline_->setLanguageChangeCallback(
      [this](const std::string &session_id, const std::string &old_lang,
  ...
  speechrnt::utils::Logger::info(
      "PipelineWebSocketIntegration initialized successfully");
  return true;
}
...
void PipelineWebSocketIntegration::shutdown() {
  ...
  active_ = false;
  speechrnt::utils::Logger::info(
      "PipelineWebSocketIntegration shutdown completed");
}

void PipelineWebSocketIntegration::handleLanguageChange(
    const std::string &session_id, const std::string &old_lang,
    const std::string &new_lang, float confidence) {
  ...
  try {
    ...
    speechrnt::utils::Logger::info(
        "Language change notification sent for session " + session_id + ": " +
        old_lang + " -> " + new_lang +
        " (confidence: " + std::to_string(confidence) + ")");
  } catch (const std::exception &e) {
    speechrnt::utils::Logger::error(
        "Failed to send language change notification: " +
        std::string(e.what()));
  }
}

void PipelineWebSocketIntegration::handleLanguageDetectionComplete(
  ...
  try {
    ...
    speechrnt::utils::Logger::debug(
        "Language detection result sent for session " + result.session_id);
  } catch (const std::exception &e) {
    speechrnt::utils::Logger::error(
        "Failed to send language detection result: " + std::string(e.what()));
  }
}

void PipelineWebSocketIntegration::handleTranslationComplete(
    const PipelineResult &result) {
  if (!active_ || !websocket_server_) {
    return;
  }
  ...
  if (result.language_changed) {
    try {
      ...
      json << "  \"detected_language\": \""
           << result.language_detection.detectedLanguage << "\",\n";
      json << "  \"language_confidence\": " << std::fixed
           << std::setprecision(3) << result.language_detection.confidence
           << "\n";
      json << "}";

      websocket_server_->sendMessage(result.session_id, json.str());
      speechrnt::utils::Logger::debug(
          "Translation complete with language change sent for session " +
          result.session_id);
    } catch (const std::exception &e) {
      speechrnt::utils::Logger::error(
          "Failed to send translation complete with language change: " +
          std::string(e.what()));
    }
  }
}

void PipelineWebSocketIntegration::handlePipelineError(
    const PipelineResult &result, const std::string &error) {
  if (!active_ || !websocket_server_) {
    return;
  }
  ...
  try {
    ...
    websocket_server_->sendMessage(result.session_id, json.str());
    speechrnt::utils::Logger::debug(
        "Pipeline error notification sent for session " + result.session_id);
  } catch (const std::exception &e) {
    speechrnt::utils::Logger::error(
        "Failed to send pipeline error notification: " + std::string(e.what()));
  }
}
...
void PipelineWebSocketIntegration::sendLanguageDetectionResult(
    const std::string &session_id, const mt::LanguageDetectionResult &result) {
  std::ostringstream json;
  json << "{\n";
