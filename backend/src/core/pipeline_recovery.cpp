...
PipelineRecovery::PipelineRecovery(std::shared_ptr<UtteranceManager> utterance_manager)
...
bool PipelineRecovery::attemptRecovery(const utils::ErrorInfo& error, uint32_t utterance_id) {
    ...
    if (recovery_it != active_recoveries_.end()) {
        ...
        if (attempt.attempt_count >= config.max_retry_attempts) {
            ...
            notifyClientRecoveryStatus(utterance_id, "Recovery failed after " + 
                                     std::to_string(config.max_retry_attempts) + " attempts", true);
            return false;
        }
    ...
    switch (config.strategy) {
        ...
        case RecoveryStrategy::NOTIFY_CLIENT_ONLY:
            notifyClientRecoveryStatus(utterance_id, "Error occurred: " + error.message, false);
...
bool PipelineRecovery::executeSkipStageRecovery(const RecoveryAttempt& attempt) {
    if (!utterance_manager_) {
        return false;
    }
...
bool PipelineRecovery::executeRestartPipelineRecovery(const RecoveryAttempt& attempt) {
    if (!utterance_manager_) {
        return false;
    }
...
std::chrono::milliseconds PipelineRecovery::calculateRetryDelay(const RecoveryAttempt& attempt) const {
    ...
    final_delay = std::min(final_delay, attempt.config.max_retry_delay.count());
...
void PipelineRecovery::recoveryWorker() {
    ...
    while (running_) {
        ...
        recovery_cv_.wait(lock, [this] { 
            return !running_ || !delayed_recovery_queue_.empty(); 
        });
        ...
        while (!delayed_recovery_queue_.empty()) {
            ...
            if (now >= target_time) {
                ...
                if (success) {
                    ...
                    notifyClientRecoveryStatus(attempt.utterance_id, "Delayed recovery successful", true);
                } else {
                    ...
                    if (attempt.attempt_count < attempt.config.max_retry_attempts) {
                        ...
                        notifyClientRecoveryStatus(attempt.utterance_id, "Delayed recovery failed", true);
                    }
                }
            } else {
