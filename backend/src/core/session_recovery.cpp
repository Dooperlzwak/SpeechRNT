...
SessionRecoveryManager::SessionRecoveryManager(
    const SessionRecoveryConfig &config)
    : config_(config) {}

SessionRecoveryManager::~SessionRecoveryManager() { shutdown(); }
...
bool SessionRecoveryManager::storeSessionData(const std::string &session_id,
                                              const SessionRecoveryData &data) {
  std::lock_guard<std::mutex> lock(sessions_mutex_);
  ...
  if (stored_sessions_.size() >= config_.max_stored_sessions) {
    ...
    auto oldest_it = std::min_element(
        stored_sessions_.begin(), stored_sessions_.end(),
        [](const auto &a, const auto &b) {
          return a.second->last_activity < b.second->last_activity;
        });

    if (oldest_it != stored_sessions_.end()) {
      speechrnt::utils::Logger::info("Removing oldest session to make room: " +
                                     oldest_it->first);
  ...
  if (!SessionRecoveryHelper::validateRecoveryData(data)) {
    speechrnt::utils::Logger::error(
        "Invalid session recovery data for session: " + session_id);
    return false;
  }
  ...
  speechrnt::utils::Logger::info("Stored session recovery data for session: " +
                                 session_id);
  return true;
}

bool SessionRecoveryManager::recoverSession(
    const std::string &session_id, std::shared_ptr<ClientSession> new_session) {
  std::lock_guard<std::mutex> lock(sessions_mutex_);

  auto it = stored_sessions_.find(session_id);
  if (it == stored_sessions_.end()) {
    speechrnt::utils::Logger::warn("No recovery data found for session: " +
                                   session_id);
    std::lock_guard<std::mutex> stats_lock(stats_mutex_);
  ...
  if (isSessionExpired(recovery_data)) {
    speechrnt::utils::Logger::warn(
        "Session recovery data expired for session: " + session_id);
  ...
  if (recovery_data.recovery_attempts >= config_.max_recovery_attempts) {
    speechrnt::utils::Logger::error(
        "Max recovery attempts exceeded for session: " + session_id);
  ...
  if (recovery_callback_) {
    try {
      ...
      speechrnt::utils::Logger::error(
          "Exception during session recovery callback: " +
          std::string(e.what()));
    ...
    recovery_success =
        SessionRecoveryHelper::applyToClientSession(recovery_data, new_session);
  }

  if (recovery_success) {
    speechrnt::utils::Logger::info("Successfully recovered session: " +
                                   session_id);
...
void SessionRecoveryManager::removeSessionData(const std::string &session_id) {
  ...
  if (it != stored_sessions_.end()) {
    ...
    speechrnt::utils::Logger::info(
        "Removed session recovery data for session: " + session_id);
  }
}

void SessionRecoveryManager::updateSessionActivity(
...
bool SessionRecoveryManager::canRecoverSession(
  ...
  return recovery_data.is_recoverable && !isSessionExpired(recovery_data) &&
         recovery_data.recovery_attempts < config_.max_recovery_attempts;
}

std::shared_ptr<SessionRecoveryData>
SessionRecoveryManager::getSessionData(const std::string &session_id) const {
  std::lock_guard<std::mutex> lock(sessions_mutex_);
...
void SessionRecoveryManager::setRecoveryCallback(
    SessionRecoveryCallback callback) {
  recovery_callback_ = callback;
}

void SessionRecoveryManager::setCleanupCallback(
    SessionCleanupCallback callback) {
  cleanup_callback_ = callback;
}

SessionRecoveryManager::RecoveryStats
SessionRecoveryManager::getRecoveryStats() const {
...
void SessionRecoveryManager::cleanupExpiredSessions() {
    ...
    if (isSessionExpired(*it->second)) {
      ...
      if (cleanup_callback_) {
        try {
          ...
          speechrnt::utils::Logger::error(
              "Exception during session cleanup callback: " +
              std::string(e.what()));
        }
      }

      speechrnt::utils::Logger::info("Cleaning up expired session: " +
                                     it->first);
  ...
  if (cleaned_count > 0) {
    ...
    speechrnt::utils::Logger::info(
        "Cleaned up " + std::to_string(cleaned_count) + " expired sessions");
  }
}

std::vector<SessionRecoveryData>
SessionRecoveryManager::exportSessionData() const {
  std::lock_guard<std::mutex> lock(sessions_mutex_);
...
void SessionRecoveryManager::cleanupWorker() {
  ...
  while (running_) {
    try {
      ...
      speechrnt::utils::Logger::error(
          "Exception in session recovery cleanup worker: " +
          std::string(e.what()));
    }
  }
...
bool SessionRecoveryManager::isSessionExpired(
    const SessionRecoveryData &data) const {
...
bool SessionRecoveryManager::loadFromStorage() {
  ...
  if (!file.is_open()) {
    speechrnt::utils::Logger::info(
        "No existing session recovery storage found");
  ...
  try {
    ...
    while (std::getline(file, line)) {
      ...
      speechrnt::utils::Logger::info(
          "Loaded session recovery data from storage");
    }

    file.close();
    return true;
  } catch (const std::exception &e) {
    speechrnt::utils::Logger::error("Failed to load session recovery data: " +
                                    std::string(e.what()));
    return false;
  }
}
...
bool SessionRecoveryManager::saveToStorage() {
  ...
  if (!file.is_open()) {
    speechrnt::utils::Logger::error(
        "Failed to open session recovery storage for writing");
    return false;
  }
  ...
  try {
    ...
    speechrnt::utils::Logger::error("Failed to save session recovery data: " +
                                    std::string(e.what()));
    return false;
  }
}
...
SessionRecoveryData SessionRecoveryHelper::createFromClientSession(
...
bool SessionRecoveryHelper::applyToClientSession(
    const SessionRecoveryData &data, std::shared_ptr<ClientSession> session) {
  ...
  try {
    ...
    speechrnt::utils::Logger::error(
        "Failed to apply recovery data to session: " + std::string(e.what()));
    return false;
  }
}

bool SessionRecoveryHelper::validateRecoveryData(
    const SessionRecoveryData &data) {
  // Basic validation
...
SessionRecoveryData
SessionRecoveryHelper::mergeRecoveryData(const SessionRecoveryData &existing,
                                         const SessionRecoveryData &update) {
...
double
SessionRecoveryHelper::calculateRecoveryScore(const SessionRecoveryData &data) {
  double score = 1.0;
