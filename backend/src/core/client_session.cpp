...
ClientSession::ClientSession(const std::string &sessionId)
    : sessionId_(sessionId), connected_(true), server_(nullptr),
      sourceLang_("en"), targetLang_("es"), vadInitialized_(false) {
  ...
  vad_->setVadCallback(
      [this](const audio::VadEvent &event) { handleVADEvent(event); });

  vad_->setUtteranceCallback(
      [this](uint32_t utteranceId, const std::vector<float> &audioData) {
...
void ClientSession::handleMessage(const std::string &message) {
  speechrnt::utils::Logger::debug("Session " + sessionId_ +
                                  " received JSON: " + message);

  if (!connected_) {
    speechrnt::utils::Logger::warn(
        "Received message for disconnected session: " + sessionId_);
    return;
  }

  // Validate and parse message using protocol
  if (!MessageProtocol::validateMessage(message)) {
    speechrnt::utils::Logger::warn("Invalid message format from session " +
                                   sessionId_);
    return;
  }
  ...
  if (!parsedMessage) {
    speechrnt::utils::Logger::warn("Failed to parse message from session " +
                                   sessionId_);
    return;
  }
  ...
  case MessageType::END_SESSION:
    processControlMessage(
        static_cast<EndSessionMessage *>(parsedMessage.get()));
    break;
  ...
  default:
    speechrnt::utils::Logger::warn(
        "Unexpected message type from client session " + sessionId_);
    break;
  }
}
...
void ClientSession::handleBinaryMessage(std::string_view data) {
  speechrnt::utils::Logger::debug(
      "Session " + sessionId_ +
      " received binary data: " + std::to_string(data.size()) + " bytes");

  if (!connected_) {
    speechrnt::utils::Logger::warn(
        "Received binary data for disconnected session: " + sessionId_);
    return;
  }
...
void ClientSession::sendMessage(const std::string &message) {
  if (connected_ && server_) {
    ...
    speechrnt::utils::Logger::warn(
        "Attempted to send message to disconnected session or no server: " +
        sessionId_);
  }
}
...
void ClientSession::sendBinaryMessage(const std::vector<uint8_t> &data) {
  if (connected_ && server_) {
    speechrnt::utils::Logger::debug(
        "Session " + sessionId_ +
        " sending binary: " + std::to_string(data.size()) + " bytes");
    server_->sendBinaryMessage(sessionId_, data);
  } else {
    speechrnt::utils::Logger::warn(
        "Attempted to send binary data to disconnected session or no server: " +
        sessionId_);
  }
}

void ClientSession::setLanguageConfig(const std::string &sourceLang,
                                      const std::string &targetLang) {
  sourceLang_ = sourceLang;
  targetLang_ = targetLang;
  speechrnt::utils::Logger::info("Session " + sessionId_ +
...
bool ClientSession::ingestAudioData(std::string_view data) {
  if (!audioIngestion_) {
    speechrnt::utils::Logger::error(
        "Audio ingestion manager not initialized for session " + sessionId_);
    return false;
  }
...
void ClientSession::setAudioFormat(const audio::AudioFormat &format) {
  if (audioIngestion_) {
    ...
    speechrnt::utils::Logger::info(
        "Session " + sessionId_ +
        " audio format updated: " + std::to_string(format.sampleRate) + "Hz, " +
...
audio::AudioIngestionManager::Statistics
ClientSession::getAudioStatistics() const {
  if (!audioIngestion_) {
    return {};
  }
...
void ClientSession::processConfigMessage(const ConfigMessage *message) {
  speechrnt::utils::Logger::info("Processing config message for session " +
                                 sessionId_);
  ...
  if (transcriptionManager_) {
    if (auto whisperSTT = dynamic_cast<::stt::WhisperSTT *>(
            transcriptionManager_->getSTTEngine())) {
      whisperSTT->setLanguageDetectionEnabled(
          message->isLanguageDetectionEnabled());
      whisperSTT->setAutoLanguageSwitching(message->isAutoLanguageSwitching());
      whisperSTT->setLanguageDetectionThreshold(
          message->getLanguageDetectionThreshold());

      speechrnt::utils::Logger::info(
          "Updated language detection settings for session " + sessionId_ +
          ": detection=" +
          (message->isLanguageDetectionEnabled() ? "enabled" : "disabled") +
          ", auto-switch=" +
          (message->isAutoLanguageSwitching() ? "enabled" : "disabled") +
          ", threshold=" +
          std::to_string(message->getLanguageDetectionThreshold()));
    }
  }
}

void ClientSession::processControlMessage(const EndSessionMessage *message) {
  speechrnt::utils::Logger::info("Processing end session message for session " +
                                 sessionId_);
...
void ClientSession::processPingMessage(const PingMessage *message) {
  speechrnt::utils::Logger::debug("Processing ping message for session " +
                                  sessionId_);
...
bool ClientSession::initializeVAD() {
  ...
  if (!vad_) {
    speechrnt::utils::Logger::error("VAD not created for session " +
                                    sessionId_);
    return false;
  }

  if (!vad_->initialize()) {
    speechrnt::utils::Logger::error("Failed to initialize VAD for session " +
                                    sessionId_ + ": " +
...
void ClientSession::setVADConfig(const audio::VadConfig &config) {
  ...
  if (vad_) {
    ...
    speechrnt::utils::Logger::info("VAD configuration updated for session " +
                                   sessionId_);
  }
}
...
void ClientSession::handleVADEvent(const audio::VadEvent &event) {
  speechrnt::utils::Logger::debug(
      "Session " + sessionId_ +
      " VAD event: " + std::to_string(static_cast<int>(event.previousState)) +
  ...
  if (event.currentState == audio::VadState::SPEECH_DETECTED) {
    if (!streamingTranscriber_) {
      if (!initializeTranscription()) {
        speechrnt::utils::Logger::error(
            "Failed to initialize transcription at speech onset for session " +
            sessionId_);
      }
    }
    if (streamingTranscriber_) {
      // Start streaming with no initial audio; audio chunks are fed in
      // processAudioData
...
void ClientSession::handleUtteranceComplete(
    uint32_t utteranceId, const std::vector<float> &audioData) {
  speechrnt::utils::Logger::info(
      "Session " + sessionId_ + " utterance " + std::to_string(utteranceId) +
  ...
  if (streamingTranscriber_) {
    ...
    // If streaming was not initialized, start and immediately finalize as
    // fallback
  ...
  if (pipelineCallback_) {
    pipelineCallback_(utteranceId, audioData, sourceLang_, targetLang_,
                      voiceId_);
  } else {
    speechrnt::utils::Logger::warn("No pipeline callback set for session " +
                                   sessionId_);
  }
}
...
void ClientSession::processAudioData(std::string_view data) {
  if (!ingestAudioData(data)) {
    speechrnt::utils::Logger::warn(
        "Session " + sessionId_ +
        " failed to ingest audio data: " + audioIngestion_->getErrorMessage());

    // Send error message to client
    ErrorMessage errorMsg("Audio ingestion failed: " +
  ...
  if (audioIngestion_) {
    ...
    if (processor) {
      ...
      auto recentSamples =
          audioIngestion_->getLatestAudio(data.size() / 2); // 16-bit samples
      ...
      if (!recentSamples.empty() && vad_) {
        ...
        if (isVADActive() &&
            vad_->getCurrentState() == audio::VadState::SPEAKING &&
            streamingTranscriber_) {
          uint32_t currentUtteranceId = vad_->getCurrentUtteranceId();
          if (currentUtteranceId != 0) {
            streamingTranscriber_->addAudioData(currentUtteranceId,
                                                recentSamples);
          }
        }
      }
  ...
  speechrnt::utils::Logger::debug(
      "Session " + sessionId_ + " ingested " + std::to_string(data.size()) +
      " bytes. " + "Total: " + std::to_string(stats.totalBytesIngested) +
      " bytes, " + std::to_string(stats.totalChunksIngested) + " chunks");
}
...
bool ClientSession::initializeTranscription() {
  ...
  if (!transcriptionManager_->initialize(modelPath, "whisper")) {
    speechrnt::utils::Logger::error(
        "Failed to initialize transcription manager for session " + sessionId_ +
  ...
  if (!streamingTranscriber_->initialize(transcriptionManager_,
                                         messageSender)) {
    speechrnt::utils::Logger::error(
        "Failed to initialize streaming transcriber for session " + sessionId_);
    return false;
  }

  // Apply normalization config from STTConfigManager if available
  if (auto engine = transcriptionManager_->getSTTEngine()) {
    // There is no direct link to STTConfigManager here; optionally could be
    // injected elsewhere. For now, read defaults from file via a lightweight
    // manager instance.
    ::stt::STTConfigManager cfgMgr;
    if (cfgMgr.loadFromFile("backend/config/stt.json")) {
      streamingTranscriber_->setNormalizationConfig(
          cfgMgr.getConfig().normalization);
    }
  }
  ...
  if (auto whisperSTT = dynamic_cast<::stt::WhisperSTT *>(
          transcriptionManager_->getSTTEngine())) {
    whisperSTT->setLanguageChangeCallback([this](const std::string &oldLang,
  ...
  speechrnt::utils::Logger::info("Transcription initialized for session " +
                                 sessionId_);
  return true;
}
...
void ClientSession::shutdownTranscription() {
  if (streamingTranscriber_) {
    ...
    speechrnt::utils::Logger::info(
        "Streaming transcriber shutdown for session " + sessionId_);
  }
  ...
  if (transcriptionManager_) {
    ...
    speechrnt::utils::Logger::info(
        "Transcription manager shutdown for session " + sessionId_);
  }
}

void ClientSession::startStreamingTranscription(
    uint32_t utteranceId, const std::vector<float> &audioData) {
  if (!streamingTranscriber_) {
    if (!initializeTranscription()) {
      speechrnt::utils::Logger::error(
          "Failed to initialize transcription for session " + sessionId_);

      // Send error message to client
      ErrorMessage errorMsg("Transcription initialization failed",
                            "TRANSCRIPTION_INIT_ERROR", utteranceId);
      sendMessage(errorMsg.serialize());
      return;
    }
  }
  ...
  speechrnt::utils::Logger::debug(
      "Started streaming transcription for utterance " +
      std::to_string(utteranceId) + " in session " + sessionId_);
}

void ClientSession::handleLanguageChange(const std::string &oldLang,
                                         const std::string &newLang,
                                         float confidence) {
  speechrnt::utils::Logger::info(
      "Session " + sessionId_ + " detected language change: " + oldLang +
      " -> " + newLang + " (confidence: " + std::to_string(confidence) + ")");
