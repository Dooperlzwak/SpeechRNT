...
size_t extractJsonSize(const std::string &json, const std::string &key,
...
const std::string GPUConfigManager::MODEL_PIPER = "piper";
...
bool GPUConfigManager::loadConfig(const std::string &configPath) {
  ...
  if (!file.is_open()) {
    Logger::warn("GPU config file not found: " + configPath +
                 ", using defaults");
    setDefaultConfigs();
    return false;
  }
...
ModelGPUConfig
GPUConfigManager::getModelConfig(const std::string &modelName) const {
...
void GPUConfigManager::setModelConfig(const std::string &modelName,
...
bool GPUConfigManager::autoDetectOptimalConfig() {
  ...
  globalConfig_.memoryLimitMB =
      std::min(static_cast<size_t>(deviceInfo.totalMemoryMB * 0.8),
               static_cast<size_t>(8192)); // Max 8GB or 80% of total
  globalConfig_.enableMemoryPool =
      deviceInfo.totalMemoryMB >= 4096; // Enable pool for 4GB+ GPUs
  globalConfig_.memoryPoolSizeMB =
      std::min(static_cast<size_t>(1024), globalConfig_.memoryLimitMB / 4);
  ...
  whisperConfig.enableQuantization =
      deviceInfo.totalMemoryMB < 6144; // Enable for <6GB GPUs
  ...
  ModelGPUConfig piperConfig;
  piperConfig.useGPU = false; // Piper runs on CPU mostly
  piperConfig.deviceId = -1;
  piperConfig.batchSize = 1;
  piperConfig.enableQuantization = false;
  piperConfig.precision = "fp32";
  setModelConfig(MODEL_PIPER, piperConfig);

  Logger::info("Auto-detected optimal GPU configuration for device: " +
               deviceInfo.name);
  return true;
}
...
bool GPUConfigManager::validateConfig() const {
  ...
  if (globalConfig_.enabled) {
    if (!isValidDeviceId(globalConfig_.deviceId)) {
      Logger::error("Invalid GPU device ID: " +
                    std::to_string(globalConfig_.deviceId));
      return false;
    }

    if (!isValidMemoryLimit(globalConfig_.memoryLimitMB)) {
      Logger::error("Invalid memory limit: " +
                    std::to_string(globalConfig_.memoryLimitMB) + "MB");
      return false;
    }
  }
  ...
  for (const auto &pair : modelConfigs_) {
    ...
    if (config.useGPU) {
      ...
      if (!isValidPrecision(config.precision)) {
        Logger::error("Invalid precision for model " + pair.first + ": " +
                      config.precision);
        return false;
      }
...
GPUConfig GPUConfigManager::getRecommendedConfig() const {
  ...
  recommended.memoryLimitMB =
      static_cast<size_t>(deviceInfo.totalMemoryMB * 0.8);
  recommended.enableMemoryPool = deviceInfo.totalMemoryMB >= 4096;
  recommended.memoryPoolSizeMB =
      std::min(static_cast<size_t>(1024), recommended.memoryLimitMB / 4);
  recommended.enableProfiling = false; // Disabled by default for performance

  return recommended;
}

std::map<std::string, ModelGPUConfig>
GPUConfigManager::getAllModelConfigs() const {
...
std::string GPUConfigManager::toJSON() const {
  ...
  json << "    \"enableMemoryPool\": "
       << (globalConfig_.enableMemoryPool ? "true" : "false") << ",\n";
  json << "    \"memoryPoolSizeMB\": " << globalConfig_.memoryPoolSizeMB
       << ",\n";
  json << "    \"enableProfiling\": "
       << (globalConfig_.enableProfiling ? "true" : "false") << "\n";
  json << "  },\n";
  ...
  for (const auto &pair : modelConfigs_) {
    ...
    json << "      \"enableQuantization\": "
         << (config.enableQuantization ? "true" : "false") << ",\n";
...
bool GPUConfigManager::fromJSON(const std::string &json) {
  try {
    ...
    if (globalStart != std::string::npos) {
      ...
      if (globalBlockStart != std::string::npos &&
          globalBlockEnd != std::string::npos) {
        std::string globalBlock = json.substr(
            globalBlockStart, globalBlockEnd - globalBlockStart + 1);
        ...
        globalConfig_.memoryLimitMB =
            extractJsonSize(globalBlock, "memoryLimitMB", 4096);
        globalConfig_.enableMemoryPool =
            extractJsonBool(globalBlock, "enableMemoryPool", true);
        globalConfig_.memoryPoolSizeMB =
            extractJsonSize(globalBlock, "memoryPoolSizeMB", 1024);
        globalConfig_.enableProfiling =
            extractJsonBool(globalBlock, "enableProfiling", false);
      }
    }
    ...
    if (modelsStart != std::string::npos) {
      ...
      std::vector<std::string> modelNames = {MODEL_WHISPER, MODEL_MARIAN,
                                             MODEL_PIPER};
      ...
      for (const auto &modelName : modelNames) {
        ...
        if (modelStart != std::string::npos) {
          ...
          if (modelBlockStart != std::string::npos &&
              modelBlockEnd != std::string::npos) {
            std::string modelBlock = json.substr(
                modelBlockStart, modelBlockEnd - modelBlockStart + 1);
            ...
            config.enableQuantization =
                extractJsonBool(modelBlock, "enableQuantization", false);
    ...
    Logger::error("Failed to parse GPU configuration JSON: " +
                  std::string(e.what()));
    setDefaultConfigs();
    return false;
  }
...
void GPUConfigManager::setDefaultConfigs() {
  ...
  modelConfigs_[MODEL_PIPER] = defaultModelConfig;
}
